---
name: icc-debug-windows
description: Manage icc debug windows and related debug menu wiring for Sidebar Debug, Background Debug, and Menu Bar Extra Debug. Use this when the user asks to open/tune these debug controls, add or adjust Debug menu entries, or capture/copy a combined debug config snapshot.
---
...
1. Verify debug menu wiring in `Sources/iccApp.swift` under `CommandMenu("Debug")`.
...
```bash
skills/icc-debug-windows/scripts/debug_windows_snapshot.sh --copy
```
5. After code edits, run build + tagged reload:
```bash
xcodebuild -project GhosttyTabs.xcodeproj -scheme icc -configuration Debug -destination 'platform=macOS' build
./scripts/reload.sh --tag <tag>
```

## Key Files

- `Sources/iccApp.swift`: Debug menu entries and debug window controllers/views.
...
Examples:
```bash
skills/icc-debug-windows/scripts/debug_windows_snapshot.sh
skills/icc-debug-windows/scripts/debug_windows_snapshot.sh --copy
skills/icc-debug-windows/scripts/debug_windows_snapshot.sh --domain <bundle-id> --copy
```
