---
name: release
description: "Prepare and ship a icc release end-to-end: choose the next version, curate user-facing changelog entries, bump versions, open and monitor a release PR, merge, tag, and verify published artifacts. Use when asked to cut, prepare, publish, or tag a new release."
---

# Release

Run this workflow to prepare and publish a icc release.
...
- **Collect contributors:** For each PR, get the author with `gh pr view <N> --repo manaflow-ai/icc --json author --jq '.author.login'`. Also check linked issue reporters with `gh issue view <N> --json author --jq '.author.login'`.
...
- `gh run watch --repo manaflow-ai/icc`
- Confirm release exists in GitHub Releases and includes `icc-macos.dmg`.
