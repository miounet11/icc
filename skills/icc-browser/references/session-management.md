...
icc uses isolated browser contexts per surface. Treat each browser surface as its own session.
...
# session B
icc browser open https://example.com --json
# -> surface:8

icc browser surface:7 get url
icc browser surface:8 get url
```
...
```bash
icc browser surface:7 state save /tmp/auth-state.json
```

### Load State

```bash
icc browser surface:8 state load /tmp/auth-state.json
...
icc browser open https://app.example.com/login --json
# login on surface:7 ...
icc browser surface:7 state save /tmp/auth.json

icc browser open https://app.example.com --json
# assume surface:8
icc browser surface:8 state load /tmp/auth.json
icc browser surface:8 goto https://app.example.com/dashboard
```
...
```bash
icc browser open https://site-a.example --json
icc browser open https://site-b.example --json
icc browser open https://site-c.example --json

icc browser surface:11 get text body > /tmp/a.txt
icc browser surface:12 get text body > /tmp/b.txt
icc browser surface:13 get text body > /tmp/c.txt
```
...
icc close-surface --surface surface:8
