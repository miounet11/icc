---
name: icc-browser
description: End-user browser automation with icc. Use when you need to open sites, interact with pages, wait for state changes, and extract data from icc browser surfaces.
---

# Browser Automation with icc

Use this skill for browser tasks inside icc webviews.
...
```bash
icc --json browser open https://example.com
# use returned surface ref, for example: surface:7

icc browser surface:7 get url
icc browser surface:7 wait --load-state complete --timeout-ms 15000
icc browser surface:7 snapshot --interactive
icc browser surface:7 fill e1 "hello"
icc --json browser surface:7 click e2 --snapshot-after
icc browser surface:7 snapshot --interactive
```
...
icc browser open https://example.com --workspace workspace:2 --window window:1 --json
```
...
icc supports wait patterns similar to agent-browser:

```bash
icc browser <surface> wait --selector "#ready" --timeout-ms 10000
icc browser <surface> wait --text "Success" --timeout-ms 10000
...
```bash
icc --json browser open https://example.com/signup
icc browser surface:7 get url
icc browser surface:7 wait --load-state complete --timeout-ms 15000
icc browser surface:7 snapshot --interactive
icc browser surface:7 fill e1 "Jane Doe"
icc browser surface:7 fill e2 "jane@example.com"
icc --json browser surface:7 click e3 --snapshot-after
icc browser surface:7 wait --url-contains "/welcome" --timeout-ms 15000
icc browser surface:7 snapshot --interactive
```
...
```bash
icc browser surface:7 fill e11 "" --snapshot-after --json
icc browser surface:7 get value e11 --json
```
...
icc browser surface:7 wait --load-state complete --timeout-ms 15000
icc browser surface:7 snapshot --interactive
icc --json browser surface:7 click e5 --snapshot-after
icc browser surface:7 snapshot --interactive
```
...
icc browser surface:7 get html body
```
