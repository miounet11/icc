...
# Absolute path
icc markdown open /path/to/PLAN.md

# Target a specific workspace
icc markdown open design.md --workspace workspace:2
```
...
When creating a plan or task list, write it to a `.md` file and open it in icc:
...
```bash
# Open in the caller's workspace (default -- uses ICC_WORKSPACE_ID)
icc markdown open plan.md

# Open in a specific workspace
icc markdown open plan.md --workspace workspace:2

# Open splitting from a specific surface
icc markdown open plan.md --surface surface:5

# Open in a specific window
icc markdown open plan.md --window window:1
```
