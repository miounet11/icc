...
If the script fails, run `say "icc release failed"`.

### 7. Verify homebrew cask

- Run `bash tests/test_homebrew_sha.sh` to confirm the cask SHA matches the release DMG
- Update the homebrew-icc submodule pointer: `git add homebrew-icc && git commit -m "Update homebrew-icc submodule to latest" && git push origin main`
