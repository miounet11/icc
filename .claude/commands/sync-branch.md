...
1. **Update submodules to latest**
   - For each submodule (ghostty, homebrew-icc, vendor/bonsplit):
     - `cd <submodule>`
     - `git fetch origin`
...
2. **Commit submodule updates on main**
   ...
   - If changed, stage and commit: `git add ghostty homebrew-icc vendor/bonsplit && git commit -m "Update submodules: <brief description>"`
