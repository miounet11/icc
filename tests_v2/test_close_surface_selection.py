...
def _wait_focused_index(client: icc, index: int, timeout: float = 4.0) -> bool:
    start = time.time()
...
def _ensure_surfaces(client: icc, count: int) -> None:
    surfaces = client.list_surfaces()
...
def test_close_middle_keeps_index(client: icc) -> TestResult:
...
def test_close_last_selects_previous(client: icc) -> TestResult:
