...
def _send_v1(command: str, *, expect_ok: bool = True) -> str:
    ...
    if expect_ok and not payload.startswith("OK"):
        raise iccError(f"{command!r} failed: {payload!r}")
    return payload


def _focused_surface_id(client: icc, workspace_id: str) -> str:
    surfaces = client.list_surfaces(workspace=workspace_id)
    for _, surface_id, focused in surfaces:
        if focused:
            return surface_id
    raise iccError(f"no focused surface in workspace {workspace_id}: {surfaces}")


def _surface_ids(client: icc, workspace_id: str) -> set[str]:
...
def _sidebar_state(workspace_id: str) -> str:
    ...
    if payload.startswith("ERROR"):
        raise iccError(f"sidebar_state failed: {payload!r}")
    return payload
...
def main() -> int:
    ...
    with icc(SOCKET_PATH) as client:
        try:
            ...
            target_directory = f"/tmp/icc-v1-report-pwd-{int(time.time() * 1000)}"
            _send_v1(
