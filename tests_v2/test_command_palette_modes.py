...
def _palette_input_selection(client: icc, window_id: str) -> dict:
...
def main() -> int:
    with icc(SOCKET_PATH) as client:
        ...
        if not switched_rows:
            raise iccError("switcher returned no rows for workspace query")
        top_id = str((switched_rows[0] or {}).get("command_id") or "")
        if not top_id.startswith("switcher."):
            raise iccError(f"expected switcher row on top for cmd+p query, got: {switched_rows[0]}")
        ...
        if not command_rows:
            raise iccError("commands mode returned no rows")
        top_command_id = str((command_rows[0] or {}).get("command_id") or "")
        if not top_command_id.startswith("palette."):
            raise iccError(f"expected command row in commands mode, got: {command_rows[0]}")
