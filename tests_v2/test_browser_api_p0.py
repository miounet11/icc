...
<!doctype html>
<html>
  <head><title>icc-browser-p0</title></head>
  <body>
    <input id='name' value=''>
...
""".strip()
        ...
        _must("icc" in str(out.get("value", "")), f"Expected #out text to include 'icc': {out}")
        ...
        _must(str(eval_res.get("value", "")) == "icc", f"Expected eval value 'icc': {eval_res}")

        snap = c._call("browser.snapshot", {"surface_id": target}) or {}
        snapshot_text = str(snap.get("snapshot") or "")
        _must("icc-browser-p0" in snapshot_text, f"Expected snapshot text to include page title: {snap}")
    ...
    print("PASS: browser.* P0 methods work on icc webview with ref handles")
    return 0
