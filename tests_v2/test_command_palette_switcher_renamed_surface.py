...
def _rename_surface(client: icc, surface_id: str, title: str) -> None:
    client._call(
        "surface.action",
        {
...
def _current_surface_id(client: icc, workspace_id: str) -> str:
...
def main() -> int:
    with icc(SOCKET_PATH) as client:
        ...
        if len(rows) < 2:
            raise iccError(f"expected at least two surfaces after split: {payload}")
        ...
        if not left_surface_id:
            raise iccError(f"failed to resolve left surface id: {payload}")
        ...
        if not result_rows:
            raise iccError("switcher returned no rows for renamed surface query")
