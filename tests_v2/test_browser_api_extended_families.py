...
def _expect_error_contains(label: str, fn, needle: str) -> None:
    try:
        ...
        if needle in text:
            return
        raise iccError(f"{label}: expected error containing {needle!r}, got: {text}")
    raise iccError(f"{label}: expected error containing {needle!r}, but call succeeded")


def _wait_selector(c: icc, surface_id: str, selector: str, timeout_s: float = 6.0) -> None:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise iccError(f"Timed out waiting for selector {selector}")


def _wait_function(c: icc, surface_id: str, expression: str, timeout_s: float = 6.0) -> None:
    timeout_ms = max(1, int(timeout_s * 1000.0))
    try:
    ...
    while time.time() < deadline:
        ...
        if bool(probe.get("value")):
            return
        time.sleep(0.05)
    raise iccError(f"Timed out waiting for function: {expression}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="icc-browser-ext-") as root:
        root_path = Path(root)
...
<html>
  <head>
    <title>icc-browser-extended-second</title>
  </head>
  <body>
...
<html>
  <head>
    <title>icc-browser-extended</title>
    <style>
  ...
  <body>
    ...
    <script>
      ...
      window.emitConsoleAndError = function () {
        console.log('icc-console-entry');
        setTimeout(function () {
          throw new Error('icc-boom');
        }, 0);
        return true;
      };
    </script>
...
def main() -> int:
    with _local_test_server() as base_url:
        ...
        with icc(SOCKET_PATH) as c:
            ...
            download_path = tempfile.NamedTemporaryFile(delete=False, prefix="icc-download-", suffix=".txt").name
            ...
            got_cookie = c._call("browser.cookies.get", {"surface_id": sid, "name": "icc_cookie"}) or {}
            cookies = got_cookie.get("cookies") or []
            _must(any(str(row.get("name")) == "icc_cookie" for row in cookies), f"Expected icc_cookie in cookies.get: {got_cookie}")
            c._call("browser.cookies.clear", {"surface_id": sid, "name": "icc_cookie"})
            got_after_clear = c._call("browser.cookies.get", {"surface_id": sid, "name": "icc_cookie"}) or {}
            ...
            c._call("browser.addinitscript", {"surface_id": sid, "script": "window.__iccInitMarker = 'init-ok';"})
            c._call("browser.navigate", {"surface_id": sid, "url": second_url})
            _wait_selector(c, sid, "#second", timeout_s=7.0)
            init_value = c._call("browser.eval", {"surface_id": sid, "script": "window.__iccInitMarker || ''"}) or {}
            ...
            state_path = tempfile.NamedTemporaryFile(delete=False, prefix="icc-state-", suffix=".json").name
