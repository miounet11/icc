...
def wait_for_notifications(client: icc, expected: int, timeout: float = 2.0) -> list[dict]:
    start = time.time()
...
def wait_for_flash_count(client: icc, surface: str, minimum: int = 1, timeout: float = 2.0) -> int:
...
def ensure_two_surfaces(client: icc) -> list[tuple[int, str, bool]]:
...
def focused_surface_index(client: icc) -> int:
    surfaces = client.list_surfaces()
...
def send_osc(client: icc, sequence: str, surface: Optional[int] = None) -> None:
...
def test_clear_prior_notifications(client: icc) -> TestResult:
...
def test_suppress_when_focused(client: icc) -> TestResult:
...
def test_not_suppressed_when_inactive(client: icc) -> TestResult:
...
def test_kitty_notification_simple(client: icc) -> TestResult:
...
def test_kitty_notification_chunked(client: icc) -> TestResult:
...
def test_rxvt_notification_osc777(client: icc) -> TestResult:
...
def test_mark_read_on_focus_change(client: icc) -> TestResult:
...
def test_mark_read_on_app_active(client: icc) -> TestResult:
...
def test_mark_read_on_tab_switch(client: icc) -> TestResult:
...
def test_flash_on_tab_switch(client: icc) -> TestResult:
...
def test_focus_on_notification_click(client: icc) -> TestResult:
...
def test_restore_focus_on_tab_switch(client: icc) -> TestResult:
...
def test_clear_on_tab_close(client: icc) -> TestResult:
