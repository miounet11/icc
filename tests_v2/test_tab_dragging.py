...
These tests use the icc socket interface to:
...
Requirements:
    - icc must be running with the socket controller enabled
"""
...
# Add the directory containing icc.py to the path
...
def ensure_focused_terminal(client: icc) -> None:
    """
...
def wait_for_terminal_in_window(client: icc, surface_idx: int, timeout: float = 5.0) -> bool:
...
def verify_terminal_responsive(client: icc, marker: Path, surface_idx: int = None, retries: int = 3) -> bool:
    """
...
def test_connection(client: icc) -> TestResult:
...
def test_initial_terminal_responsive(client: icc) -> TestResult:
    """Test that the initial terminal is responsive."""
    result = TestResult("Initial Terminal Responsive")
    marker = Path(tempfile.gettempdir()) / f"icc_init_{os.getpid()}"
...
def test_split_right_responsive(client: icc) -> TestResult:
    """Test that both terminals remain responsive after horizontal split."""
    result = TestResult("Split Right - Both Responsive")
    marker0 = Path(tempfile.gettempdir()) / f"icc_split0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"icc_split1_{os.getpid()}"
...
def test_split_down_responsive(client: icc) -> TestResult:
    """Test that both terminals remain responsive after vertical split."""
    result = TestResult("Split Down - Both Responsive")
    marker0 = Path(tempfile.gettempdir()) / f"icc_splitv0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"icc_splitv1_{os.getpid()}"
...
def test_focus_switching(client: icc) -> TestResult:
    """Test that focus switching between panes works correctly."""
    result = TestResult("Focus Switching")
    markers = [
        Path(tempfile.gettempdir()) / f"icc_focus{i}_{os.getpid()}"
        for i in range(3)
    ]
...
def test_split_ratio_50_50(client: icc) -> TestResult:
    """Test that splits create 50/50 pane ratios."""
    result = TestResult("Split Ratio 50/50")
    cols_file_0 = Path(tempfile.gettempdir()) / f"icc_cols0_{os.getpid()}"
    cols_file_1 = Path(tempfile.gettempdir()) / f"icc_cols1_{os.getpid()}"
...
def test_new_surfaces(client: icc) -> TestResult:
    """Test creating new surfaces in a pane."""
    result = TestResult("New Surfaces")
    markers = [
        Path(tempfile.gettempdir()) / f"icc_bonsplit{i}_{os.getpid()}"
        for i in range(3)
    ]
...
def test_pane_commands(client: icc) -> TestResult:
    """Test the new pane commands (list_panes, focus_pane)."""
    result = TestResult("Pane Commands")
    marker = Path(tempfile.gettempdir()) / f"icc_pane_{os.getpid()}"
...
def test_close_horizontal_split(client: icc) -> TestResult:
    """Test that closing one side of a horizontal split preserves the other terminal."""
    result = TestResult("Close Horizontal Split")
    marker0 = Path(tempfile.gettempdir()) / f"icc_close_h0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"icc_close_h1_{os.getpid()}"
...
def test_close_vertical_split(client: icc) -> TestResult:
    """Test that closing one side of a vertical split preserves the other terminal."""
    result = TestResult("Close Vertical Split")
    marker0 = Path(tempfile.gettempdir()) / f"icc_close_v0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"icc_close_v1_{os.getpid()}"
...
def test_close_first_pane_vertical_split(client: icc) -> TestResult:
    ...
    marker0 = Path(tempfile.gettempdir()) / f"icc_close_fv0_{os.getpid()}"
    marker1 = Path(tempfile.gettempdir()) / f"icc_close_fv1_{os.getpid()}"
...
def test_close_nested_splits(client: icc) -> TestResult:
    """Test closing splits in a nested configuration."""
    result = TestResult("Close Nested Splits")
    markers = [
        Path(tempfile.gettempdir()) / f"icc_nested_{i}_{os.getpid()}"
        for i in range(4)
    ]
...
def test_rapid_split_close_vertical(client: icc) -> TestResult:
    ...
    marker = Path(tempfile.gettempdir()) / f"icc_rapid_{os.getpid()}"
...
def test_rapid_split_close_first_pane(client: icc) -> TestResult:
    ...
    marker = Path(tempfile.gettempdir()) / f"icc_rapid_first_{os.getpid()}"
...
def run_tests():
    """Run all tests."""
    print("=" * 60)
    print("icc Tab Dragging E2E Tests")
    print("=" * 60)
    print()
    ...
    socket_path = icc.DEFAULT_SOCKET_PATH
    if not os.path.exists(socket_path):
        print(f"Error: Socket not found at {socket_path}")
        print("Please make sure icc is running.")
        return 1
