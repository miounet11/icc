    ...
    for attempt in range(1, retries + 1):
        ...
        if proc.returncode == 0:
            try:
                ...
                raise iccError(f"Invalid CLI JSON output for {' '.join(args)}: {proc.stdout!r} ({exc})")
        ...
        if "Command timed out" in merged and attempt < retries:
            time.sleep(0.2)
            continue
        raise iccError(f"CLI failed ({' '.join(args)}): {merged}")

    raise iccError(f"CLI failed ({' '.join(args)}): {last_merged}")
...
def _run_cli_expect_failure(cli: str, args: list[str], needles: list[str]) -> None:
    ...
    if proc.returncode == 0:
        raise iccError(f"Expected CLI failure for {' '.join(args)}, but it succeeded: {proc.stdout}")
    merged = f"{proc.stdout}\n{proc.stderr}"
    if not any(needle in merged for needle in needles):
        raise iccError(f"Expected CLI failure containing one of {needles!r} for {' '.join(args)}, got: {merged}")


@contextmanager
def _local_test_server() -> str:
    with tempfile.TemporaryDirectory(prefix="icc-browser-cli-") as root:
        root_path = Path(root)
...
def main() -> int:
    ...
    with _local_test_server() as page_url:
        ...
        os.environ["ICC_WORKSPACE_ID"] = workspace
        ...
        state_file = tempfile.NamedTemporaryFile(delete=False, prefix="icc-cli-state-", suffix=".json").name
