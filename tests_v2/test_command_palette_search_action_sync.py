...
def main():
    with icc(SOCKET_PATH) as client:
        ...
        if not before_rows:
            raise iccError(f"no results for 'open': {before}")
        if str(before_rows[0].get("command_id") or "") != "palette.terminalOpenDirectory":
            raise iccError(f"unexpected top command for 'open': {before_rows[0]}")
        ...
        if not after_rows:
            raise iccError(f"no results for 'rename' after replacement: {after}")
        top_after = str(after_rows[0].get("command_id") or "")
        if top_after not in {"palette.renameWorkspace", "palette.renameTab"}:
            raise iccError(f"top result did not update to rename command after replacement: {after_rows[0]}")
