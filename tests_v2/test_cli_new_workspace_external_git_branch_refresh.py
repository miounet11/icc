...
def _resolve_socket_path() -> str:
    socket_path = os.environ.get("ICC_SOCKET", "").strip()
    if not socket_path:
        raise iccError("ICC_SOCKET is required (expected /tmp/icc-debug-<tag>.sock)")
    if not re.fullmatch(r"/tmp/icc-debug-[^/]+\.sock", socket_path):
        raise iccError(f"ICC_SOCKET must be a tagged debug socket, got: {socket_path!r}")
    return socket_path
...
def _find_cli_binary() -> str:
    ...
    candidates = glob.glob(
        os.path.expanduser("~/Library/Developer/Xcode/DerivedData/**/Build/Products/Debug/icc"),
        recursive=True,
    )
...
def _create_git_repo(root: Path) -> Path:
    ...
    subprocess.run(
        ["git", "config", "user.email", "icc-test@example.com"],
        cwd=repo,
        check=True,
        stdout=subprocess.DEVNULL,
        stderr=subprocess.DEVNULL,
...
def main() -> int:
    ...
    temp_root = Path(tempfile.mkdtemp(prefix="icc_issue_915_external_git_"))
