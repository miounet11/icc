...
def _open_server() -> tuple[str, socketserver.TCPServer, threading.Thread, tempfile.TemporaryDirectory[str]]:
    root = tempfile.TemporaryDirectory(prefix="icc-browser-cli-logs-")
  ...
  <body>
    ...
    <script>
      window.emitLogs = function () {
        ...
        setTimeout(function () { throw new Error('icc-browser-boom'); }, 0);
        return true;
      };
    </script>
  </body>
...
def main() -> int:
    ...
    try:
        with icc(SOCKET_PATH) as c:
            ...
            _must("icc-console-entry" in console_output, f"browser console text mode should print entries: {console_output!r}")
            ...
            _must("icc-browser-boom" in errors_output, f"browser errors text mode should print entries: {errors_output!r}")
