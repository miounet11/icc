...
if [ "$(id -un)" != "icc" ]; then
  ...
  echo "Run via: ssh icc-vm 'cd /Users/icc/GhosttyTabs && ./scripts/run-tests-v2.sh'" >&2
  exit 2
fi

cd "$(dirname "$0")/.."

DERIVED_DATA_PATH="$HOME/Library/Developer/Xcode/DerivedData/icc-tests-v2"
...
launch_and_wait() {
  ...
  SOCK=""
  for _ in {1..120}; do
    SOCK=$(ls -t /tmp/icc-debug*.sock /tmp/icc*.sock 2>/dev/null | head -1 || true)
  ...
  if [ -z "$SOCK" ] || [ ! -S "$SOCK" ]; then
    echo "ERROR: Socket not ready (looked for /tmp/icc*.sock)" >&2
    exit 1
  fi
  export ICC_SOCKET_PATH="$SOCK"
  export ICC_SOCKET="$SOCK"

  # Ensure LaunchServices has a visible/main window attached for rendering checks.
  ICC_TAG="$RUN_TAG" open "$APP" >/dev/null 2>&1 || true
  sleep 0.5
...
from icc import icc  # type: ignore
