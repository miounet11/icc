...
SOCKET_PATH="/tmp/icc-debug.sock"
STABILITY_WAIT=15

echo "=== Smoke Test ==="

# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/icc DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/icc DEV"
if [ ! -x "$BINARY" ]; then
...
rm -f "$SOCKET_PATH"
pkill -x "icc DEV" 2>/dev/null || true
sleep 1

# --- Launch the app directly (not via `open`, which can silently fail on CI) ---
echo "Launching app..."
ICC_SOCKET_MODE=allowAll ICC_UI_TEST_MODE=1 "$BINARY" > /tmp/icc-smoke-stdout.log 2>&1 &
APP_PID=$!
...
if ! kill -0 "$APP_PID" 2>/dev/null; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/icc-smoke-stdout.log 2>/dev/null | tail -50 || true
  echo "--- debug log ---"
  tail -50 /tmp/icc-debug.log 2>/dev/null || true
  echo "--- crash reports ---"
  ls -lt ~/Library/Logs/DiagnosticReports/*icc* 2>/dev/null | head -5 || echo "(none)"
  exit 1
fi
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/icc-smoke-stdout.log 2>/dev/null | tail -30 || true
  echo "--- debug log ---"
  tail -30 /tmp/icc-debug.log 2>/dev/null || true
  ls -la /tmp/icc-debug* 2>/dev/null || true
  pgrep -la "icc" || echo "No icc processes found"
  exit 1
fi
