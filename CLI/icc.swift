...
private final class CLISocketSentryTelemetry {
    ...
    private static func currentSentryBundleIdentifier() -> String? {
        if let bundleIdentifier = ProcessInfo.processInfo.environment["ICC_BUNDLE_ID"]?
    ...
    func breadcrumb(_ message: String, data: [String: Any] = [:]) {
        ...
        let crumb = Breadcrumb(level: .info, category: "icc.cli")
    ...
    func captureError(stage: String, error: Error) {
        ...
        _ = SentrySDK.capture(error: error) { scope in
            ...
            scope.setTag(value: "icc-cli", key: "component")
...
final class SocketClient {
    ...
    private static let responseTimeoutSeconds: TimeInterval = {
        ...
        if let raw = env["ICC_CLI_RESPONSE_TIMEOUT_SEC"],
    ...
    static func waitForFilesystemPath(_ path: String, timeout: TimeInterval) throws {
        ...
        let queue = DispatchQueue(label: "com.icc.cli.path-watch.\(UUID().uuidString)")
        let semaphore = DispatchSemaphore(value: 0)
        var found = false
        let source = DispatchSource.makeFileSystemObjectSource(
            fileDescriptor: watchFD,
...
struct ICCCLI {
    ...
    func run() throws {
        ...
        case "identify":
            ...
            if includeCaller {
                ...
                let workspaceArg = idWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
                let surfaceArg = optionValue(commandArgs, name: "--surface") ?? (idWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
        ...
        case "new-split":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
            let surfaceRaw = sfArg ?? panelArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
        ...
        case "close-surface":
            ...
            let workspaceArg = csWsFlag ?? (windowId == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
            let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? optionValue(commandArgs, name: "--panel") ?? (csWsFlag == nil && windowId == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
            var params: [String: Any] = [:]
            let wsId = try normalizeWorkspaceHandle(workspaceArg, client: client)
            if let wsId { params["workspace_id"] = wsId }
            let sfId = try normalizeSurfaceHandle(surfaceRaw, client: client, workspaceHandle: wsId)
        ...
        case "trigger-flash":
            ...
            let callerWorkspaceArg = preferTTYFallback
                ? nil
                : (windowId == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
            ...
            let callerSurfaceArg = explicitSurfaceArg == nil && preferTTYFallback == false && windowId == nil
                ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"]
                : nil
        ...
        case "read-screen":
            ...
            let workspaceArg = wsArg ?? (windowId == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowId == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
        ...
        case "clear-notifications":
            ...
            if let wsFlag = optionValue(commandArgs, name: "--workspace") {
                ...
                socketCmd += " --tab=\(wsId)"
                      ...
                      let envWs = ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"],
    ...
    private func runMarkdownCommand(
        ...
        let workspaceRaw = workspaceOpt ?? (windowOpt == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
        if let workspaceRaw {
    ...
    /// Open a path in icc by creating a new workspace with the given directory.
    ...
    private func runFeedback(
        ...
        if emailOpt == nil && bodyOpt == nil && imagePaths.isEmpty {
            var params: [String: Any] = [:]
            let env = ProcessInfo.processInfo.environment
            if let workspaceId = env["ICC_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines),
               !workspaceId.isEmpty {
    ...
    private func runWorkspaceAction(
        ...
        let workspaceArg = workspaceOpt ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
    ...
    private func runTabAction(
        ...
        let tabArg = tabOpt
            ...
            ?? (workspaceOpt == nil && windowOverride == nil
                ? (ProcessInfo.processInfo.environment["ICC_TAB_ID"] ?? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"])
                : nil)
    ...
    private func buildSSHBootstrapCommandSnippet(
        ...
        let remoteCommandBase64Placeholder = "__ICC_REMOTE_BOOTSTRAP_B64_RUNTIME__"
        ...
        var lines: [String] = [
            "icc_workspace_id=\"${ICC_WORKSPACE_ID:-}\"",
            "icc_surface_id=\"${ICC_SURFACE_ID:-}\"",
            "icc_remote_bootstrap_b64=\(shellQuote(encodedBootstrapScript))",
            "icc_remote_bootstrap=\"$(printf %s \"$icc_remote_bootstrap_b64\" | base64 -d 2>/dev/null || printf %s \"$icc_remote_bootstrap_b64\" | base64 -D 2>/dev/null)\"",
            "icc_remote_bootstrap=\"$(printf '%s' \"$icc_remote_bootstrap\" | sed \"s/__ICC_WORKSPACE_ID__/$icc_workspace_id/g; s/__ICC_SURFACE_ID__/$icc_surface_id/g\")\"",
            "icc_remote_bootstrap_b64_runtime=\"$(printf '%s' \"$icc_remote_bootstrap\" | base64 | tr -d '\\n')\"",
            "icc_remote_command_template=\(shellQuote(remoteCommandTemplate))",
            "icc_remote_command=\"$(printf '%s' \"$icc_remote_command_template\" | sed \"s|\(remoteCommandBase64Placeholder)|$icc_remote_bootstrap_b64_runtime|g\")\"",
        ]

        var sshInvocation = "command \(sshPrefix) -o \"RemoteCommand=$icc_remote_command\""
        if !hasSSHOptionKey(options.sshOptions, key: "RequestTTY") {
            sshInvocation += " -tt"
        }
    ...
    func buildInteractiveRemoteShellScript(
        ...
        let remoteCallerExportLines = [
            "if [ -n '__ICC_WORKSPACE_ID__' ]; then export ICC_WORKSPACE_ID='__ICC_WORKSPACE_ID__'; fi",
        ...
        outerLines += [
            "ICCZSHENV",
            "    cat > \"$icc_shell_dir/.zprofile\" <<'ICCZSHPROFILE'",
        ]
        outerLines.append(contentsOf: zshProfileLines)
        outerLines += [
            "ICCZSHPROFILE",
            "    cat > \"$icc_shell_dir/.zshrc\" <<'ICCZSHRC'",
        ]
        outerLines.append(contentsOf: zshRCLines)
        outerLines += [
            "ICCZSHRC",
            "    cat > \"$icc_shell_dir/.zlogin\" <<'ICCZSHLOGIN'",
        ]
        outerLines.append(contentsOf: zshLoginLines)
        outerLines += [
            "ICCZSHLOGIN",
            "    chmod 600 \"$icc_shell_dir/.zshenv\" \"$icc_shell_dir/.zprofile\" \"$icc_shell_dir/.zshrc\" \"$icc_shell_dir/.zlogin\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    export ICC_REAL_ZDOTDIR=\"${ZDOTDIR:-$HOME}\"",
            "    export ZDOTDIR=\"$icc_shell_dir\"",
        ...
        outerLines += [
            "ICCBASHRC",
            "    chmod 600 \"$icc_shell_dir/.bashrc\" >/dev/null 2>&1 || true",
        ]
        outerLines.append(contentsOf: relayWarmupLines.map { "    " + $0 })
        outerLines += [
            "    exec \"$ICC_LOGIN_SHELL\" --rcfile \"$icc_shell_dir/.bashrc\" -i",
            "    ;;",
            "  *)",
        ]
    ...
    func buildSSHStartupCommand(
        ...
        scriptLines += [
            "ICC_SSH_SESSION_ENDED=0",
            "icc_ssh_session_end() { if [ \"${ICC_SSH_SESSION_ENDED:-0}\" = 1 ]; then return; fi; ICC_SSH_SESSION_ENDED=1; \(lifecycleCleanup); }",
            "trap 'icc_ssh_session_end' EXIT HUP INT TERM",
        ]
        if isShellSnippet {
    ...
    private func buildSSHSessionEndShellCommand(remoteRelayPort: Int) -> String {
        [
            "if [ -n \"${ICC_BUNDLED_CLI_PATH:-}\" ]",
            "&& [ -x \"${ICC_BUNDLED_CLI_PATH}\" ]",
            "&& [ -n \"${ICC_SOCKET_PATH:-}\" ]",
            "&& [ -n \"${ICC_WORKSPACE_ID:-}\" ]",
    ...
    private func runSSHSessionEnd(commandArgs: [String], client: SocketClient) throws {
        ...
        let workspaceRaw = optionValue(commandArgs, name: "--workspace") ?? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"]
        let surfaceRaw = optionValue(commandArgs, name: "--surface") ?? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"]
        guard let workspaceRaw,
              let workspaceId = try normalizeWorkspaceHandle(workspaceRaw, client: client),
              !workspaceId.isEmpty else {
            throw CLIError(message: "ssh-session-end requires --workspace or ICC_WORKSPACE_ID")
        }
        guard let surfaceRaw,
            ...
            throw CLIError(message: "ssh-session-end requires --surface or ICC_SURFACE_ID")
        }
    ...
    private func runRemoteDaemonStatus(commandArgs: [String], jsonOutput: Bool) throws {
        ...
        let payload: [String: Any] = [
            ...
            "dev_local_build_fallback": ProcessInfo.processInfo.environment["ICC_REMOTE_DAEMON_ALLOW_LOCAL_BUILD"] == "1",
        ...
        if manifest == nil {
            print("note: this build has no embedded remote daemon manifest. Set ICC_REMOTE_DAEMON_ALLOW_LOCAL_BUILD=1 only for dev builds.")
        }
    }
    ...
    private func remoteDaemonManifest() -> RemoteDaemonManifest? {
        for plistURL in candidateInfoPlistURLs() {
            guard let raw = NSDictionary(contentsOf: plistURL) as? [String: Any],
                  let rawManifest = raw["ICCRemoteDaemonManifestJSON"] as? String,
...
#if DEBUG
        let trimmedExplicit = ProcessInfo.processInfo.environment["ICC_DEBUG_LOG"]?
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let path: String? = {
    ...
    private func runBrowserCommand(
        ...
        func displayBrowserValue(_ value: Any) -> String {
            if let dict = value as? [String: Any],
               let type = dict["__icc_t"] as? String,
               type == "undefined" {
                return "undefined"
            }
            if value is NSNull {
        ...
        if subcommand == "open" || subcommand == "open-split" || subcommand == "new" {
            ...
            let respectExternalOpenRules: Bool = {
                guard let raw = ProcessInfo.processInfo.environment["ICC_RESPECT_EXTERNAL_OPEN_RULES"] else {
                    return false
                }
    ...
    private func subcommandUsage(_ command: String) -> String? {
        ...
        case "identify":
            ...
            Flags:
              --workspace <id|ref|index>   Caller workspace context (default: $ICC_WORKSPACE_ID)
              --surface <id|ref|index>     Caller surface context (default: $ICC_SURFACE_ID)
        ...
        case "workspace-action":
            ...
            Flags:
              ...
              --workspace <id|ref|index>   Target workspace (default: current/$ICC_WORKSPACE_ID)
        ...
        case "tab-action":
            ...
            Flags:
              --action <name>              Action name (required if not positional)
              --tab <id|ref|index>         Target tab (accepts tab:<n> or surface:<n>; default: $ICC_TAB_ID, then $ICC_SURFACE_ID, then focused tab)
              --surface <id|ref|index>     Alias for --tab (backward compatibility)
              --workspace <id|ref|index>   Workspace context (default: current/$ICC_WORKSPACE_ID)
              --title <text>               Title for rename (or pass trailing title text)
        ...
        case "rename-tab":
            ...
            1) --tab
            2) --surface
            3) $ICC_TAB_ID / $ICC_SURFACE_ID
            4) currently focused tab (optionally within --workspace)

            Flags:
              --workspace <id|ref>   Workspace context (default: current/$ICC_WORKSPACE_ID)
        ...
        case "new-split":
            ...
            Flags:
              ...
              --surface <id|ref>     Surface to split from (default: $ICC_SURFACE_ID)
        ...
        case "close-surface":
            ...
            Flags:
              --surface <id|ref>     Surface to close (default: $ICC_SURFACE_ID)
              --panel <id|ref>       Alias for --surface
              --workspace <id|ref>   Workspace context (default: $ICC_WORKSPACE_ID)
        ...
        case "capture-pane":
            ...
            Flags:
              --workspace <id|ref>   Workspace context (default: $ICC_WORKSPACE_ID)
              --surface <id|ref>     Surface context (default: $ICC_SURFACE_ID)
        ...
        case "browser":
            ...
            Subcommands:
              open|open-split|new [url] [--workspace <id|ref|index>] [--window <id|ref|index>]
                open/open-split/new default to $ICC_WORKSPACE_ID when --workspace is omitted and --window is not set
        ...
        case "markdown":
            ...
            Options:
              --workspace <id|ref|index>   Target workspace (default: $ICC_WORKSPACE_ID)
    ...
    private func runInteractiveThemes() throws {
        ...
        if let light = selection.light {
            environment["ICC_THEME_PICKER_INITIAL_LIGHT"] = light
        }
        if let dark = selection.dark {
            environment["ICC_THEME_PICKER_INITIAL_DARK"] = dark
        }
    ...
    private func printThemesList(jsonOutput: Bool) throws {
        ...
        let configPath = try iccThemeOverrideConfigURL().path
    ...
    private func themeConfigSearchURLs() -> [URL] {
        let rawPaths = [
            ...
            "~/Library/Application Support/\(Self.iccThemeOverrideBundleIdentifier)/config",
            "~/Library/Application Support/\(Self.iccThemeOverrideBundleIdentifier)/config.ghostty",
        ]
    ...
    private func iccThemeOverrideConfigURL() throws -> URL {
        guard let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first else {
            throw CLIError(message: "Unable to resolve Application Support directory")
        }
        return appSupport
            .appendingPathComponent(Self.iccThemeOverrideBundleIdentifier, isDirectory: true)
            .appendingPathComponent("config.ghostty", isDirectory: false)
    }

    private func writeManagedThemeOverride(rawThemeValue: String) throws -> URL {
        let fileManager = FileManager.default
        let configURL = try iccThemeOverrideConfigURL()
        ...
        let strippedContents = removingManagedThemeOverride(from: existingContents)
            .trimmingCharacters(in: .whitespacesAndNewlines)
        let block = """
        \(Self.iccThemesBlockStart)
        theme = \(rawThemeValue)
        \(Self.iccThemesBlockEnd)
        """
    ...
    private func workspaceFromArgsOrEnv(_ args: [String], windowOverride: String? = nil) -> String? {
        ...
        return ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"]
    }
    ...
    private func treeCallerContextFromEnvironment() -> [String: Any]? {
        let env = ProcessInfo.processInfo.environment
        let workspaceRaw = env["ICC_WORKSPACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        let surfaceRaw = env["ICC_SURFACE_ID"]?.trimmingCharacters(in: .whitespacesAndNewlines)
        var caller: [String: Any] = [:]
    ...
    private func tmuxCallerWorkspaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"])
    }
    ...
    private func tmuxCallerPaneHandle() -> String? {
        guard let pane = normalizedTmuxTarget(ProcessInfo.processInfo.environment["TMUX_PANE"])
            ?? normalizedTmuxTarget(ProcessInfo.processInfo.environment["ICC_PANE_ID"]) else {
            return nil
        }
    ...
    private func tmuxCallerSurfaceHandle() -> String? {
        normalizedTmuxTarget(ProcessInfo.processInfo.environment["ICC_SURFACE_ID"])
    }
    ...
    private func isIccClaudeWrapper(at path: String) -> Bool {
    ...
    private func resolveClaudeExecutable(searchPath: String?) -> String? {
        ...
        for entry in entries where !entry.isEmpty {
            ...
            guard !isIccClaudeWrapper(at: candidate) else { continue }
            return candidate
        }
        return nil
    }
    ...
    private func configureClaudeTeamsEnvironment(
        ...
        let fakeTmuxPane = focusedContext.map { "%\($0.paneHandle)" }
            ...
            ?? "%1"
        let fakeTerm = processEnvironment["ICC_CLAUDE_TEAMS_TERM"] ?? "screen-256color"

        setenv("CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS", "1", 1)
        setenv("ICC_CLAUDE_TEAMS_ICC_BIN", executablePath, 1)
        setenv("PATH", updatedPath, 1)
        setenv("TMUX", fakeTmuxValue, 1)
        setenv("TMUX_PANE", fakeTmuxPane, 1)
        setenv("TERM", fakeTerm, 1)
        setenv("ICC_SOCKET_PATH", socketPath, 1)
        setenv("ICC_SOCKET", socketPath, 1)
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
            setenv("ICC_SOCKET_PASSWORD", explicitPassword, 1)
        }
        unsetenv("TERM_PROGRAM")
        if let focusedContext {
            setenv("ICC_WORKSPACE_ID", focusedContext.workspaceId, 1)
            if let surfaceId = focusedContext.surfaceId, !surfaceId.isEmpty {
                setenv("ICC_SURFACE_ID", surfaceId, 1)
            }
        }
    }
    ...
    private func runClaudeTeams(
        ...
        launcherEnvironment["ICC_SOCKET_PATH"] = socketPath
        launcherEnvironment["ICC_SOCKET"] = socketPath
        if let explicitPassword,
           !explicitPassword.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
    ...
    private func runTmuxCompatCommand(
        ...
        case "capture-pane":
            ...
            let (sfArg, rem1) = parseOption(rem0, name: "--surface")
            let (linesArg, rem2) = parseOption(rem1, name: "--lines")
            let workspaceArg = wsArg ?? (windowOverride == nil ? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"] : nil)
            let surfaceArg = sfArg ?? (wsArg == nil && windowOverride == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
    ...
    private func runClaudeHook(
        ...
        let workspaceArg = hookWsFlag ?? ProcessInfo.processInfo.environment["ICC_WORKSPACE_ID"]
        let surfaceArg = optionValue(hookArgs, name: "--surface") ?? (hookWsFlag == nil ? ProcessInfo.processInfo.environment["ICC_SURFACE_ID"] : nil)
        ...
        case "session-start", "active":
            ...
            let claudePid: Int? = {
                guard let raw = ProcessInfo.processInfo.environment["ICC_CLAUDE_PID"]?
                    .trimmingCharacters(in: .whitespacesAndNewlines),
                    let pid = Int(raw),
                    pid > 0 else {
                    return nil
    ...
    private func resolveCallerTTYName() -> String? {
        let env = ProcessInfo.processInfo.environment
        for key in ["ICC_CLI_TTY_NAME", "ICC_TTY_NAME", "TTY", "SSH_TTY"] {
    ...
    private func resolvedVersionInfo() -> [String: String] {
        ...
        if info["ICCCommit"] == nil,
           let commit = normalizedCommitHash(ProcessInfo.processInfo.environment["ICC_COMMIT"]) {
            info["ICCCommit"] = commit
        }
    ...
    private func versionInfo(from dictionary: [String: Any]?) -> [String: String]? {
        ...
        if let commit = dictionary["ICCCommit"] as? String,
           let normalizedCommit = normalizedCommitHash(commit) {
            info["ICCCommit"] = normalizedCommit
        }
    ...
    private func usage() -> String {
        ...
        Socket Auth:
          --password takes precedence, then ICC_SOCKET_PASSWORD env var, then password saved in Settings.
...
struct ICCTermMain {
    static func main() {
        ...
        let cli = ICCCLI(args: CommandLine.arguments)
        do {
            try cli.run()
        } catch {
