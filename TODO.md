...
- Older tasks below may still mention `icc`, `ICC_*`, or `iccd-remote` because those identifiers remain in the implementation and migration backlog.

## Issue 151: Remote SSH (Living Execution)
- [x] `icc ssh` creates remote workspace metadata and does not require `--name`
- [x] Remote daemon bootstrap/upload/start path with `iccd-remote serve --stdio`
...
- [ ] Extend `iccd-remote` RPC beyond `hello/ping` with proxy stream methods (`proxy.open|close`)
...
- [x] Add browser automation API inspired by `vercel-labs/agent-browser`, but backed by icc's WKWebView (wait, click, type, eval, screenshot, etc.).
- [x] Finalize browser parity contract and command mapping decisions in `docs/agent-browser-port-spec.md`.
- [x] Add `icc browser` command surface that mirrors agent-browser semantics and targets explicit `surface_id` handles.
...
- [x] Fix `icc browser open|open-split|new` URL parsing so routing flags (`--workspace`, `--window`) are removed before URL construction.
...
- [x] Expand end-user skill docs with deep-linkable icc-browser references/templates plus a new core `skills/icc/` topology skill.
...
- [ ] Add "Install Claude Code integration" menu item in menubar
  ...
  - Implement as part of `icc` CLI, menubar just triggers the CLI command
...
- [ ] Cmd+click should open links in icc (browser panel) instead of external browser
...
- [x] Extend `icc browser ...` CLI grammar for the new families (including aliases).
...
- [x] Re-run full `run-tests-v1.sh` and `run-tests-v2.sh` on `icc-vm`.
