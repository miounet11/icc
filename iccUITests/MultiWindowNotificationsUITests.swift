...
final class MultiWindowNotificationsUITests: XCTestCase {
    ...
    override func setUp() {
        ...
        dataPath = "/tmp/icc-ui-test-multi-window-notifs-\(UUID().uuidString).json"
        socketPath = "/tmp/icc-ui-test-socket-\(UUID().uuidString).sock"
    ...
    func testNotificationsRouteToCorrectWindow() {
        ...
        app.launchEnvironment["ICC_UI_TEST_MULTI_WINDOW_NOTIF_SETUP"] = "1"
        app.launchEnvironment["ICC_UI_TEST_MULTI_WINDOW_NOTIF_PATH"] = dataPath
        app.launchEnvironment["ICC_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
    ...
    func testEmptyNotificationsPopoverBlocksTerminalTyping() throws {
        ...
        app.launchEnvironment["ICC_SOCKET_PATH"] = socketPath
        app.launchEnvironment["ICC_SOCKET_MODE"] = "allowAll"
        app.launchEnvironment["ICC_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["ICC_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["ICC_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let marker = "icc_notif_block_\(UUID().uuidString.replacingOccurrences(of: "-", with: "").prefix(8))"
    ...
    func testNotifyCLIDoesNotStealFocusAcrossWindows() throws {
        ...
        app.launchEnvironment["ICC_SOCKET_ENABLE"] = "1"
        app.launchEnvironment["ICC_UI_TEST_SOCKET_SANITY"] = "1"
        app.launchEnvironment["ICC_UI_TEST_NOTIFY_SOURCE_TERMINAL_READY"] = "1"
        app.launchEnvironment["ICC_UI_TEST_ENABLE_DUPLICATE_LAUNCH_OBSERVER"] = "1"
        app.launchEnvironment["ICC_TAG"] = launchTag
        app.launch()
        XCTAssertTrue(
            ensureForegroundAfterLaunch(app, timeout: 12.0),
        ...
        let commandStatusPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ui-test-notify-\(commandResultStem).status")
            .path
        let commandStdoutPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ui-test-notify-\(commandResultStem).stdout")
            .path
        let commandStderrPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ui-test-notify-\(commandResultStem).stderr")
            .path
        let commandScriptPath = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ui-test-notify-\(commandResultStem).sh")
            .path
        defer {
        ...
        guard let bundledCLIPath = resolveIccCLIPaths(strategy: .bundledOnly).first else {
            XCTFail("Failed to locate bundled icc CLI for notify regression test")
            return
        }
        ...
        do {
            ...
            XCTFail(
                "Failed to write delayed bundled `icc notify` script. " +
        ...
        XCTAssertTrue(
            ...
            "Expected icc to move to background before delayed notify command runs. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertTrue(
            ...
            "Expected delayed bundled `icc notify` command to finish without foregrounding icc. state=\(app.state.rawValue)"
        )
        ...
        XCTAssertFalse(
            ...
            "Expected icc to remain in background after bundled `icc notify`. state=\(app.state.rawValue) stderr=\(notifyStderr)"
        )
        guard notifyExitStatus == "0" else {
            XCTFail(
                "Expected bundled `icc notify` launched from the in-app shell to succeed. " +
    ...
    private func waitForIccPing(timeout: TimeInterval) -> (stdout: String?, stderr: String?) {
        var lastStdout: String?
        var lastStderr: String?
        let didSucceed = waitForCondition(timeout: timeout) {
            let result = self.runIccCommand(
        ...
        let result = runIccCommand(socketPath: socketPath, arguments: ["ping"], responseTimeoutSeconds: 2.0)
        let stdout = result.stdout.isEmpty ? nil : result.stdout
        let stderr = result.stderr.isEmpty ? nil : result.stderr
    ...
    private func runIccCommand(
        socketPath: String,
        arguments: [String],
        responseTimeoutSeconds: Double = 3.0,
        cliStrategy: IccCLIStrategy = .any
    ) -> (terminationStatus: Int32, stdout: String, stderr: String) {
        var args = ["--socket", socketPath]
        args.append(contentsOf: arguments)
        var environment = ProcessInfo.processInfo.environment
        environment["ICC_CLI_RESPONSE_TIMEOUT_SEC"] = String(responseTimeoutSeconds)

        let cliPaths = resolveIccCLIPaths(strategy: cliStrategy)
        if cliPaths.isEmpty, cliStrategy == .bundledOnly {
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to locate bundled icc CLI"
            )
        }

        var lastPermissionFailure: (terminationStatus: Int32, stdout: String, stderr: String)?
        for cliPath in cliPaths {
            let result = executeIccCommand(
        ...
        if cliStrategy == .bundledOnly {
            return lastPermissionFailure ?? (
                terminationStatus: -1,
                stdout: "",
                stderr: "Bundled icc CLI command failed without an executable path"
            )
        }

        let fallbackArgs = ["icc"] + args
        let fallbackResult = executeIccCommand(
    ...
    private enum IccCLIStrategy: Equatable {
        case any
        case bundledOnly
    }
    ...
    private func resolveIccCLIPaths(strategy: IccCLIStrategy) -> [String] {
        ...
        if strategy == .any {
            for key in ["ICC_UI_TEST_CLI_PATH", "ICC_CLI"] {
        ...
        candidates.append("/tmp/icc-\(launchTag)/Build/Products/Debug/icc DEV.app/Contents/Resources/bin/icc")
        candidates.append("/tmp/icc-\(launchTag)/Build/Products/Debug/icc.app/Contents/Resources/bin/icc")
        if strategy == .any {
            candidates.append("/tmp/icc-\(launchTag)/Build/Products/Debug/icc")
        }
    ...
    private func appendCLIPathCandidates(
        ...
        strategy: IccCLIStrategy,
        to candidates: inout [String]
    ) {
        candidates.append("\(productsDir)/icc DEV.app/Contents/Resources/bin/icc")
        candidates.append("\(productsDir)/icc.app/Contents/Resources/bin/icc")
        if strategy == .any {
            candidates.append("\(productsDir)/icc")
        }
        ...
        for entry in entries.sorted() where entry.hasSuffix(".app") {
            let cliPath = URL(fileURLWithPath: productsDir)
                ...
                .appendingPathComponent("Contents/Resources/bin/icc")
                .path
            candidates.append(cliPath)
        }
        if strategy == .any {
            for entry in entries.sorted() where entry == "icc" {
                let cliPath = URL(fileURLWithPath: productsDir)
                    .appendingPathComponent(entry)
                    .path
                candidates.append(cliPath)
    ...
    private func executeIccCommand(
        ...
        do {
            ...
            return (
                terminationStatus: -1,
                stdout: "",
                stderr: "Failed to run icc command: \(error.localizedDescription) (cliPath=\(executablePath))"
            )
        }
    ...
    private func resolveSocketPath(timeout: TimeInterval, requiredWorkspaceId: String? = nil) -> String? {
        ...
        _ = waitForCondition(timeout: timeout) {
            for candidate in primaryCandidates {
                ...
                // Primary candidate is the explicitly requested ICC_SOCKET_PATH. If it responds,
    ...
    private func expectedSocketCandidates(includeGlobalFallback: Bool) -> [String] {
        ...
        let taggedDebugSocket = "/tmp/icc-debug-\(launchTag).sock"
        ...
        if includeGlobalFallback {
            ...
            candidates.append("/tmp/icc-debug.sock")
            candidates.append(stableSocketPath())
            candidates.append("/tmp/icc.sock")
        }
    ...
    private func stableSocketPath() -> String {
        FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first?
            ...
            .appendingPathComponent("icc.sock", isDirectory: false)
            .path ?? "/tmp/icc.sock"
    }
    ...
    private func discoverTmpSocketCandidates(limit: Int) -> [String] {
        ...
        let matches = entries.filter { $0.hasPrefix("icc") && $0.hasSuffix(".sock") }
