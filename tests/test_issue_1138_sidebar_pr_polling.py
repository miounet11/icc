...
def _git_stub() -> str:
    return textwrap.dedent(
        ...
        if [ "$1" = "remote" ] && [ "$2" = "get-url" ] && [ "$3" = "origin" ]; then
          printf 'https://github.com/manaflow-ai/icc.git\\n'
          exit 0
        fi
...
def _gh_stub() -> str:
    return textwrap.dedent(
        """\
        #!/bin/sh
        args_log="${ICC_TEST_GH_ARGS_LOG:?}"
        count_file="${ICC_TEST_GH_COUNT_FILE:?}"
        pid_file="${ICC_TEST_GH_PID_FILE:-}"
        scenario="${ICC_TEST_SCENARIO:?}"
        head_file="${ICC_TEST_HEAD_FILE:?}"
        ...
        case "$scenario" in
          prompt_helper_idle)
            printf '1138\\tOPEN\\thttps://github.com/manaflow-ai/icc/pull/1138\\n'
            ;;
          ...
          branch_switch_clear)
            if [ "$branch" = "feature/old" ]; then
              printf '111\\tOPEN\\thttps://github.com/manaflow-ai/icc/pull/111\\n'
              exit 0
            fi
...
def _shell_command(kind: str, scenario: str) -> str:
    ...
    if kind == "zsh":
        return textwrap.dedent(
            f"""\
            source "$ICC_TEST_SCRIPT"
            _icc_send() {{ print -r -- "$1" >> "$ICC_TEST_SEND_LOG"; }}
            _icc_prompt_entry() {{ _icc_precmd; }}
            _icc_cleanup() {{ _icc_zshexit; }}
            {shared}"""
        )
    ...
    if kind == "bash":
        return textwrap.dedent(
            ...
            source "$ICC_TEST_SCRIPT"
            _icc_send() {{ printf '%s\\n' "$1" >> "$ICC_TEST_SEND_LOG"; }}
            _icc_prompt_entry() {{ _icc_prompt_command; }}
            _icc_cleanup() {{ type _icc_bash_cleanup >/dev/null 2>&1 && _icc_bash_cleanup; }}
            {shared}"""
        )
...
def _report_line(number: int) -> str:
    return (
        f"report_pr {number} https://github.com/manaflow-ai/icc/pull/{number} "
...
def _run_case(base: Path, *, shell: str, shell_args: list[str], script: Path, scenario: str) -> tuple[int, str]:
    ...
    env["ICC_SOCKET_PATH"] = str(socket_path)
    env["ICC_TAB_ID"] = "00000000-0000-0000-0000-000000000001"
    env["ICC_PANEL_ID"] = "00000000-0000-0000-0000-000000000002"
    env["ICC_TEST_SCRIPT"] = str(script)
    env["ICC_TEST_REPO"] = str(repo)
    env["ICC_TEST_SEND_LOG"] = str(send_log)
    env["ICC_TEST_GH_COUNT_FILE"] = str(gh_count_file)
    env["ICC_TEST_GH_ARGS_LOG"] = str(gh_args_log)
    env["ICC_TEST_GH_PID_FILE"] = str(gh_pid_file)
    env["ICC_TEST_SCENARIO"] = scenario
    env["ICC_TEST_HEAD_FILE"] = str(head_file)
    env["ICC_TEST_HANG_SECONDS"] = "4"
...
def main() -> int:
    ...
    cases = [
        ("zsh", ["-f", "-c"], root / "Resources" / "shell-integration" / "icc-zsh-integration.zsh"),
        ("bash", ["--noprofile", "--norc", "-c"], root / "Resources" / "shell-integration" / "icc-bash-integration.bash"),
    ]
    scenarios = [
        "prompt_helper_idle",
        "transient_same_context",
    ...
    base = Path("/tmp") / f"icc_issue_1138_pr_poll_{os.getpid()}"
    try:
