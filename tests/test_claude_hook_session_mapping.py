...
def run_claude_hook(
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"icc claude-hook {subcommand} failed:\n"
            f"exit={proc.returncode}\nstdout={proc.stdout}\nstderr={proc.stderr}"
        )
    return proc.stdout.strip()


def wait_for_notification_count(client: icc, minimum: int, timeout: float = 4.0) -> list[dict]:
    start = time.time()
    items: list[dict] = []
...
def main() -> int:
    ...
    state_path = Path(tempfile.gettempdir()) / f"icc_claude_hook_state_{os.getpid()}.json"
    ...
    project_dir = Path(tempfile.gettempdir()) / f"icc_claude_map_project_{os.getpid()}"
    ...
    try:
        with icc() as client:
            ...
            hook_env["ICC_SOCKET_PATH"] = client.socket_path
            hook_env["ICC_WORKSPACE_ID"] = workspace_id
            hook_env["ICC_SURFACE_ID"] = surface_id
            hook_env["ICC_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
    ...
    except (iccError, RuntimeError) as exc:
        return fail(str(exc))
    finally:
        try:
            if state_path.exists():
