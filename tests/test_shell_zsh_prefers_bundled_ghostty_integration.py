#!/usr/bin/env python3
"""
Regression: the icc zsh wrapper should prefer a bundled Ghostty zsh
integration file in ICC_SHELL_INTEGRATION_DIR over the fallback integration
under GHOSTTY_RESOURCES_DIR.

Without this, tagged icc builds can silently load Ghostty's installed app
...
def main() -> int:
    ...
    base = Path("/tmp") / f"icc_bundled_ghostty_zsh_{os.getpid()}"
    try:
        ...
        env["ICC_SHELL_INTEGRATION_DIR"] = str(bundled)
        env["GHOSTTY_RESOURCES_DIR"] = str(fallback)
        env["ICC_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        env["ICC_SHELL_INTEGRATION"] = "0"
        env["ICC_TEST_OUT"] = str(marker)
