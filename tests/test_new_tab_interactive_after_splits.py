...
def _wait_for_terminal_focus(c: icc, panel_id: str, timeout_s: float = 8.0) -> None:
    start = time.time()
    while time.time() - start < timeout_s:
        try:
            c.activate_app()
...
def _wait_for_text(c: icc, panel_id: str, needle: str, timeout_s: float = 2.5) -> None:
    start = time.time()
    last = ""
    while time.time() - start < timeout_s:
    ...
    raise iccError(f"Timed out waiting for token in terminal text: {needle}\nLast tail:\n{tail}")


def _type_and_wait_visible(c: icc, panel_id: str, cmd: str) -> bool:
...
def _wait_for_tmp_write(c: icc, panel_id: str, tmp: str, token: str) -> None:
...
def main() -> int:
    with icc(SOCKET_PATH) as c:
        ...
        panes = c.list_panes()
        if len(panes) < 2:
            raise iccError(f"expected multiple panes, got: {panes}")
        ...
        for i in range(6):
            ...
            token = f"ICC_NEW_TAB_OK_{i}_{uuid.uuid4().hex[:10]}"
            tmp = f"/tmp/icc_new_tab_{token}.txt"
