...
# Catches issues like https://github.com/manaflow-ai/icc/issues/110 where a race
# condition caused the cask to contain the SHA of a 404 page instead of the DMG.
set -euo pipefail

CASK_FILE="$(dirname "$0")/../homebrew-icc/Casks/icc.rb"

if [ ! -f "$CASK_FILE" ]; then
  echo "SKIP: homebrew-icc submodule not initialized"
  exit 0
fi
...
URL="https://github.com/manaflow-ai/icc/releases/download/v${VERSION}/icc-macos.dmg"
TMPFILE=$(mktemp)
