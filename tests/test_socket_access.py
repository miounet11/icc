...
In iccOnly mode (default), only processes descended from the icc
app process can connect. External processes (e.g., SSH) are rejected.

Test strategy:
  Phase 1: iccOnly — external processes get rejected
  Phase 2: iccOnly — internal process CAN connect (inject via shell rc)
...
def _preferred_worktree_slug():
    env_slug = os.environ.get("ICC_TAG") or os.environ.get("ICC_BRANCH_SLUG")
    if env_slug:
...
def _derived_app_candidates_for_current_worktree():
    ...
    for info_path in info_paths:
        ...
        app_path = os.path.join(derived_root, "Build/Products/Debug/icc DEV.app")
...
def _find_app():
    explicit = os.environ.get("ICC_APP_PATH")
    ...
    if preferred_slug:
        preferred_tmp = []
        preferred_tmp.extend(glob.glob(f"/tmp/icc-{preferred_slug}/Build/Products/Debug/icc DEV*.app"))
        preferred_tmp.extend(glob.glob(f"/private/tmp/icc-{preferred_slug}/Build/Products/Debug/icc DEV*.app"))
    ...
    derived_candidates = glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/icc DEV.app"
    ))
    tmp_candidates = []
    tmp_candidates.extend(glob.glob("/tmp/icc-*/Build/Products/Debug/icc DEV*.app"))
    tmp_candidates.extend(glob.glob("/private/tmp/icc-*/Build/Products/Debug/icc DEV*.app"))
...
def _find_cli(preferred_app_path: str = ""):
    ...
    if preferred_app_path:
        ...
        sibling = os.path.join(debug_dir, "icc")
    ...
    candidates.extend(glob.glob(os.path.join(
        home, "Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/icc"
    )))
    candidates.extend(glob.glob("/tmp/icc-*/Build/Products/Debug/icc"))
    candidates.extend(glob.glob("/private/tmp/icc-*/Build/Products/Debug/icc"))
...
def _kill_icc(app_path: str = None):
    if app_path:
        exe = os.path.join(app_path, "Contents/MacOS/icc DEV")
        subprocess.run(["pkill", "-f", exe], capture_output=True)
    else:
        subprocess.run(["pkill", "-x", "icc DEV"], capture_output=True)
    time.sleep(1.5)


def _launch_icc(app_path: str, socket_path: str, mode: str = None, extra_env: dict = None):
    if os.path.exists(socket_path):
        try:
    ...
    env_args = []
    if mode:
        env_args = ["--env", f"ICC_SOCKET_MODE={mode}"]
    launch_env = {
        "ICC_SOCKET_PATH": socket_path,
...
def test_internal_process_allowed(socket_path: str, app_path: str) -> TestResult:
    """
    Verify a icc-spawned terminal process CAN connect in iccOnly mode.
    Inject a test via the shell rc file, then launch icc in iccOnly mode.
    The shell (a descendant of icc) runs the test on startup.
    """
    result = TestResult("Internal process can connect (iccOnly)")
    marker = os.path.join(tempfile.gettempdir(), f"icc_internal_{os.getpid()}")
    hook_file = os.path.join(tempfile.gettempdir(), f"icc_rc_hook_{os.getpid()}.sh")
...
""")
        ...
        # Kill existing icc, launch in iccOnly mode (default)
        _kill_icc(app_path)
        _launch_icc(app_path, socket_path, mode="iccOnly")
        ...
        if content == "OK":
            result.success("Internal process pinged socket successfully in iccOnly mode")
        else:
...
def test_allowall_mode_works(socket_path: str, app_path: str) -> TestResult:
    """Verify ICC_SOCKET_MODE=allowAll bypasses ancestry check."""
    result = TestResult("allowAll mode allows external")
    try:
        _kill_icc(app_path)
        _launch_icc(app_path, socket_path, mode="allowAll")
...
def test_password_mode_requires_auth(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        _launch_icc(
            ...
            socket_path,
            mode="password",
            extra_env={"ICC_SOCKET_PASSWORD": password}
        )
...
def test_password_mode_cli_exit_code(socket_path: str, app_path: str) -> TestResult:
    ...
    try:
        ...
        if not cli_path:
            result.failure("Could not find icc CLI binary")
            return result
...
def run_tests():
    print("=" * 60)
    print("icc Socket Access Control Tests")
    print("=" * 60)
    print()

    app_path = _find_app()
    if not app_path:
        print("Error: Could not find icc DEV.app in DerivedData")
        return 1
    print(f"App: {app_path}")

    socket_path = f"/tmp/icc-test-socket-access-{os.getpid()}.sock"
    try:
        os.unlink(socket_path)
    except OSError:
        pass
    ...
    # ── Phase 1: iccOnly — external rejection ──
    print("Phase 1: iccOnly mode — external rejection")
    print("-" * 50)

    # Ensure icc is running in iccOnly mode
    _kill_icc(app_path)
    print("  Launching icc in iccOnly mode...")
    _launch_icc(app_path, socket_path, mode="iccOnly")
    ...
    # ── Phase 2: iccOnly — internal process CAN connect ──
    print("Phase 2: iccOnly mode — internal process allowed")
    print("-" * 50)
    ...
    # ── Cleanup: leave icc in iccOnly mode ──
    _kill_icc(app_path)
    _launch_icc(app_path, socket_path, mode="iccOnly")
