#!/usr/bin/env python3
"""
Regression test: `icc claude-teams` injects the tmux-style auto-mode env.
"""
...
from claude_teams_test_utils import resolve_icc_cli
...
def main() -> int:
    try:
        cli_path = resolve_icc_cli()
    except Exception as exc:
        print(f"FAIL: {exc}")
        return 1

    with tempfile.TemporaryDirectory(prefix="icc-claude-teams-env-") as td:
        tmp = Path(td)
...
printf '%s\\n' "${ICC_CLAUDE_TEAMS_ICC_BIN-__UNSET__}" > "$FAKE_ICC_BIN_LOG"
printf '%s\\n' "$@" > "$FAKE_ARGV_LOG"
...
printf '%s\\n' "${ICC_SOCKET_PATH-__UNSET__}" > "$FAKE_SOCKET_PATH_LOG"
printf '%s\\n' "${ICC_SOCKET_PASSWORD-__UNSET__}" > "$FAKE_SOCKET_PASSWORD_LOG"
""",
        )
        ...
        env["FAKE_ICC_BIN_LOG"] = str(icc_bin_log)
        ...
        explicit_socket_path = str(tmp / "explicit-icc.sock")
        ...
        if proc.returncode != 0:
            print("FAIL: `icc claude-teams --version` exited non-zero")
        ...
        if tmux_path.startswith(str(real_bin)):
            print(f"FAIL: expected icc tmux shim to shadow PATH, got {tmux_path!r}")
            return 1

        icc_bin_value = read_text(icc_bin_log)
        if not icc_bin_value or icc_bin_value == "__UNSET__":
            print("FAIL: missing ICC_CLAUDE_TEAMS_ICC_BIN")
            return 1

        if not os.path.exists(icc_bin_value):
            print(f"FAIL: ICC_CLAUDE_TEAMS_ICC_BIN does not exist: {icc_bin_value!r}")
            return 1
        ...
        if socket_path_value != explicit_socket_path:
            print(f"FAIL: expected ICC_SOCKET_PATH={explicit_socket_path!r}, got {socket_path_value!r}")
            return 1

        socket_password_value = read_text(socket_password_log)
        if socket_password_value != explicit_socket_password:
            print(
                "FAIL: expected ICC_SOCKET_PASSWORD to preserve the explicit CLI override, "
                f"got {socket_password_value!r}"
            )
            return 1

    print("PASS: icc claude-teams injects the auto-mode tmux env and shim")
    return 0
