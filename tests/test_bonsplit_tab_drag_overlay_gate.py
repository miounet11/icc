...
def wait_for_overlay_probe_ready(client: icc, timeout_s: float = 8.0) -> None:
    start = time.time()
    last_error = None
    while time.time() - start < timeout_s:
        try:
            ...
            last_error = e
            time.sleep(0.1)
    raise iccError(f"overlay_hit_gate probe unavailable: {last_error}")


def assert_gate(client: icc, event_type: str, expected: bool, reason: str) -> None:
...
def assert_drop_gate(client: icc, source: str, expected: bool, reason: str) -> None:
...
def assert_portal_gate(client: icc, event_type: str, expected: bool, reason: str) -> None:
...
def assert_sidebar_gate(client: icc, state: str, expected: bool, reason: str) -> None:
...
def main() -> int:
    ...
    if not os.path.exists(socket_path):
        ...
        print("Tip: start icc first (or set ICC_TAG / ICC_SOCKET_PATH).")
        return 0
