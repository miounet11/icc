...
if ! grep -Fq 'com.icc.app.nightly.universal' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly workflow must set a distinct .universal bundle ID"
  exit 1
fi

if ! grep -Fq 'https://github.com/manaflow-ai/icc/releases/download/nightly/appcast-universal.xml' "$WORKFLOW_FILE"; then
...
if ! awk '
  ...
  in_upload && /icc-nightly-macos\*\.dmg/ { saw_arm_artifacts=1 }
  in_upload && /icc-nightly-universal-macos\*\.dmg/ { saw_universal_artifacts=1 }
...
if ! awk '
  ...
  in_publish && /icc-nightly-universal-macos-\$\{\{ github\.run_id \}\}\*\.dmg/ { saw_universal_immutable=1 }
  in_publish && /icc-nightly-universal-macos\.dmg/ { saw_universal_stable=1 }
