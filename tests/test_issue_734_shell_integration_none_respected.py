...
icc wrapper .zshenv should only source Ghostty zsh integration when Ghostty
...
def _run_case(
    *,
    ...
    env["ICC_TEST_OUT"] = str(out_path)

    # Keep input deterministic and local to this test.
    for key in (
        "GHOSTTY_ZSH_ZDOTDIR",
        "ICC_ZSH_ZDOTDIR",
        "ICC_ORIGINAL_ZDOTDIR",
    ...
    if ghostty_enabled:
        ...
        env["ICC_ZSH_ZDOTDIR"] = str(orig_zdotdir)
...
def main() -> int:
    ...
    base = Path("/tmp") / f"icc_issue_734_{os.getpid()}"
    try:
