...
def v2_call(client: icc, method: str, params: dict[str, Any] | None = None, request_id: str = "1") -> dict[str, Any]:
    payload = {
        "id": request_id,
        "method": method,
        "params": params or {},
    ...
    try:
        ...
        raise iccError(f"Invalid v2 JSON response for {method}: {raw}") from exc

    if not parsed.get("ok"):
        raise iccError(f"v2 {method} failed: {parsed.get('error')}")
...
def browser_address_bar_focus_state(client: icc, surface_id: str | None = None, request_id: str = "browser-focus") -> dict[str, Any]:
    params: dict[str, Any] = {}
    if surface_id:
...
def set_command_palette_visible(client: icc, window_id: str, target_visible: bool) -> bool:
    for idx in range(5):
        state = v2_call(
            client,
...
def command_palette_results(client: icc, window_id: str, limit: int = 20) -> list[dict[str, Any]]:
    payload = v2_call(
        client,
...
def command_palette_selected_index(client: icc, window_id: str) -> int:
    payload = v2_call(
        client,
...
def move_command_palette_selection_to_index(client: icc, window_id: str, target_index: int) -> bool:
...
def main() -> int:
    ...
    try:
        ...
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to ensure command palette is hidden for scenario 1")
        ...
        if not terminal_id:
            raise iccError("Missing terminal surface for focus setup")
        ...
        if not did_focus_address_bar:
            raise iccError("Blank browser surface did not focus omnibar after focus_surface")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to ensure command palette is hidden for scenario 2")
        ...
        if not left_terminal_id:
            raise iccError("Missing initial terminal surface for split setup")
        ...
        if not left_pane or not browser_pane:
            raise iccError("Failed to locate split panes for pane-focus scenario")
        ...
        if not did_focus_split_browser:
            raise iccError("Blank browser pane did not focus omnibar after focus_pane")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to reset command palette before scenario 3")
        ...
        if not terminal_id:
            raise iccError("Missing terminal surface for command palette scenario")
        ...
        if not set_command_palette_visible(client, window_id, True):
            raise iccError("Failed to open command palette")
        ...
        if not palette_visible_after_focus:
            raise iccError("Command palette closed unexpectedly after focus_surface")
        ...
        if bool(blank_focus_state.get("focused")):
            raise iccError("Blank browser tab stole omnibar focus while command palette was visible")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to reset command palette before scenario 4 (target setup)")
        ...
        if not did_focus_target_browser:
            raise iccError("Failed to focus omnibar on target workspace browser before Cmd+P switch")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to reset command palette before scenario 4 (source setup)")
        ...
        if not source_terminal_id:
            raise iccError("Missing terminal surface for Cmd+P workspace switcher scenario")
        ...
        if not wait_for(
            ...
            raise iccError("Cmd+P did not open command palette switcher")
        ...
        if has_surface_rows:
            raise iccError("Cmd+P switcher listed unexpected surface rows; expected workspace-only results")
        ...
        if target_index is None:
            raise iccError(f"Cmd+P switcher did not list target workspace command {target_command_id}")

        if not move_command_palette_selection_to_index(client, window_id, target_index):
            raise iccError(f"Failed to move Cmd+P selection to result index {target_index}")
        ...
        if not did_focus_switcher_target:
            raise iccError("Cmd+P workspace switch did not restore blank browser omnibar focus")
        ...
        window_id = current_window_id(client)
        if not set_command_palette_visible(client, window_id, False):
            raise iccError("Failed to reset command palette before scenario 5")
        ...
        if not wait_for(
            ...
            raise iccError("Cmd+P did not open command palette switcher before Escape scenario")
        ...
        if not did_dismiss_switcher_on_escape:
            raise iccError("Cmd+P Escape did not dismiss command palette switcher")
