...
def run_wrapper(*, socket_state: str, argv: list[str]) -> tuple[int, list[str], list[str], str, str]:
    with tempfile.TemporaryDirectory(prefix="icc-claude-wrapper-test-") as td:
        tmp = Path(td)
...
""",
        ...
        make_executable(
            ...
            """#!/usr/bin/env bash
set -euo pipefail
printf '%s timeout=%s\\n' "$*" "${ICC_CLI_RESPONSE_TIMEOUT_SEC-__UNSET__}" >> "$FAKE_ICC_LOG"
if [[ "${1:-}" == "--socket" ]]; then
  shift 2
fi
if [[ "${1:-}" == "ping" ]]; then
  if [[ "${FAKE_ICC_PING_OK:-0}" == "1" ]]; then
    exit 0
  fi
  exit 1
fi
...
""",
        ...
        return proc.returncode, read_lines(real_args_log), read_lines(icc_log), proc.stderr.strip(), claudecode_value
...
def test_live_socket_injects_supported_hooks(failures: list[str]) -> None:
    code, real_argv, icc_log, stderr, claudecode = run_wrapper(socket_state="live", argv=["hello"])
    ...
    expect(any(" ping" in line for line in icc_log), f"live socket: expected icc ping, got {icc_log}", failures)
    expect(
        any("timeout=0.75" in line for line in icc_log),
        f"live socket: expected bounded ping timeout, got {icc_log}",
        failures,
    )
...
def test_missing_socket_skips_hook_injection(failures: list[str]) -> None:
    code, real_argv, icc_log, stderr, claudecode = run_wrapper(socket_state="missing", argv=["hello"])
    ...
    expect(icc_log == [], f"missing socket: expected no icc calls, got {icc_log}", failures)
...
def test_stale_socket_skips_hook_injection(failures: list[str]) -> None:
    code, real_argv, icc_log, stderr, claudecode = run_wrapper(socket_state="stale", argv=["hello"])
    ...
    expect(any(" ping" in line for line in icc_log), f"stale socket: expected icc ping probe, got {icc_log}", failures)
    expect(
        any("timeout=0.75" in line for line in icc_log),
        f"stale socket: expected bounded ping timeout, got {icc_log}",
        failures,
    )
