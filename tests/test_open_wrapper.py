...
def run_wrapper(
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="icc-open-wrapper-test-") as td:
        tmp = Path(td)
        wrapper = tmp / "open"
...
key="${3:-}"
case "$key" in
  browserInterceptTerminalOpenCommandInIccBrowser)
    ...
    exit 1
    ;;
  browserOpenTerminalLinksInIccBrowser)
...
if [[ -n "${FAKE_ICC_FAIL_URLS:-}" ]]; then
  IFS=',' read -r -a failures <<< "$FAKE_ICC_FAIL_URLS"
...
""",
        ...
        env = os.environ.copy()
        env["ICC_SOCKET_PATH"] = "/tmp/icc-open-wrapper-test.sock"
        env["ICC_BUNDLE_ID"] = "com.icc.app.debug.test"
        env["ICC_OPEN_WRAPPER_SYSTEM_OPEN"] = str(system_open)
        env["ICC_OPEN_WRAPPER_DEFAULTS"] = str(defaults)
        env["FAKE_OPEN_LOG"] = str(open_log)
        env["FAKE_ICC_LOG"] = str(icc_log)
        if python_bin is None:
            env.pop("ICC_OPEN_WRAPPER_PYTHON3", None)
        else:
            env["ICC_OPEN_WRAPPER_PYTHON3"] = python_bin
        ...
        if fail_urls:
            env["FAKE_ICC_FAIL_URLS"] = ",".join(fail_urls)
        else:
            env.pop("FAKE_ICC_FAIL_URLS", None)
        ...
        return read_log(open_log), read_log(icc_log), result.returncode, result.stderr.strip()
...
def test_toggle_disabled_passthrough(failures: list[str]) -> None:
    ...
    open_log, icc_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="0",
        whitelist="",
    )
    expect(code == 0, f"toggle off: wrapper exited {code}: {stderr}", failures)
    expect(icc_log == [], f"toggle off: icc should not be called, got {icc_log}", failures)
...
def test_toggle_disabled_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [],
        f"toggle off (case-insensitive): icc should not be called, got {icc_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_whitelist_miss_passthrough(failures: list[str]) -> None:
    ...
    expect(icc_log == [], f"whitelist miss: icc should not be called, got {icc_log}", failures)
    expect(open_log == [url], f"whitelist miss: expected system open [{url}], got {open_log}", failures)


def test_whitelist_match_routes_to_icc(failures: list[str]) -> None:
    ...
    expect(icc_log == [f"browser open {url}"], f"whitelist match: unexpected icc log {icc_log}", failures)
...
def test_external_literal_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [f"browser open {url}"],
        f"external literal deferred: expected wrapper to pass URL to icc, got {icc_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_pattern_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [f"browser open {url}"],
        f"external regex deferred: expected wrapper to pass URL to icc, got {icc_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_regex_with_icu_features_is_deferred_to_app(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [f"browser open {url}"],
        f"external regex icu deferred: expected wrapper to pass URL to icc, got {icc_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_external_invalid_regex_is_ignored_silently(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [f"browser open {url}"],
        f"external invalid regex: expected icc open for {url}, got {icc_log}",
        failures,
    )
    expect(
        open_log == [],
...
def test_partial_failures_only_fallback_failed_urls(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [f"browser open {good}", f"browser open {failed}"],
        f"partial failure: icc log mismatch {icc_log}",
        failures,
    )
    expect(
...
def test_legacy_toggle_fallback_passthrough(failures: list[str]) -> None:
    ...
    expect(icc_log == [], f"legacy fallback: icc should not be called, got {icc_log}", failures)
...
def test_legacy_toggle_fallback_case_insensitive_passthrough(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [],
        f"legacy fallback (case-insensitive): icc should not be called, got {icc_log}",
        failures,
    )
    expect(
        open_log == [url],
...
def test_uppercase_scheme_routes_to_icc(failures: list[str]) -> None:
    url = "HTTPS://api.example.com/path?q=1"
    open_log, icc_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(icc_log == [f"browser open {url}"], f"uppercase scheme: unexpected icc log {icc_log}", failures)


def test_local_html_file_routes_to_icc(failures: list[str]) -> None:
    ...
    expect(len(icc_log) == 1, f"local html file: expected exactly one icc call, got {icc_log}", failures)
    if icc_log:
        expect(
            icc_log[0].startswith("browser open file://"),
            f"local html file: expected file:// target, got {icc_log[0]}",
            failures,
        )
        expect(
            "hello%20page.HTML" in icc_log[0],
            f"local html file: expected URL-encoded filename in icc target, got {icc_log[0]}",
            failures,
        )


def test_file_url_html_routes_to_icc(failures: list[str]) -> None:
    url = "file:///tmp/icc-open-wrapper-fixture.html"
    open_log, icc_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
        whitelist="",
    ...
    expect(icc_log == [f"browser open {url}"], f"file url html: unexpected icc log {icc_log}", failures)


def test_file_url_html_routes_to_icc_without_python_binary(failures: list[str]) -> None:
    url = "file:///tmp/icc-open-wrapper-fixture.html"
    open_log, icc_log, code, stderr = run_wrapper(
        args=[url],
        intercept_setting="1",
    ...
    expect(
        icc_log == [f"browser open {url}"],
        f"file url html no-python fallback: unexpected icc log {icc_log}",
        failures,
    )


def test_local_html_file_routes_to_icc_without_python_binary(failures: list[str]) -> None:
    ...
    expect(
        len(icc_log) == 1,
        f"local html no-python fallback: expected exactly one icc call, got {icc_log}",
        failures,
    )
    if icc_log:
        expect(
            icc_log[0].startswith("browser open file://"),
            f"local html no-python fallback: expected file:// target, got {icc_log[0]}",
            failures,
        )
        expect(
            "no%20python%20fallback.html" in icc_log[0],
            f"local html no-python fallback: expected URL-encoded filename, got {icc_log[0]}",
            failures,
        )
...
def test_domain_like_html_argument_passthrough(failures: list[str]) -> None:
    ...
    expect(
        icc_log == [],
        f"domain-like html argument: icc should not be called, got {icc_log}",
        failures,
    )
    expect(
        open_log == [arg],
...
def test_non_file_scheme_html_passthrough(failures: list[str]) -> None:
    ...
    expect(icc_log == [], f"non-file scheme html: icc should not be called, got {icc_log}", failures)
...
def test_mailto_html_passthrough(failures: list[str]) -> None:
    ...
    expect(icc_log == [], f"mailto html: icc should not be called, got {icc_log}", failures)
...
def test_local_non_html_file_passthrough(failures: list[str]) -> None:
    ...
    expect(icc_log == [], f"local non-html file: icc should not be called, got {icc_log}", failures)
...
def test_unicode_whitelist_matches_punycode_url(failures: list[str]) -> None:
    ...
    expect(icc_log == [f"browser open {url}"], f"unicode whitelist: unexpected icc log {icc_log}", failures)
...
def test_punycode_whitelist_matches_unicode_url(failures: list[str]) -> None:
    ...
    expect(icc_log == [f"browser open {url}"], f"punycode whitelist: unexpected icc log {icc_log}", failures)
...
def main() -> int:
    ...
    test_whitelist_match_routes_to_icc(failures)
    ...
    test_uppercase_scheme_routes_to_icc(failures)
    test_local_html_file_routes_to_icc(failures)
    test_file_url_html_routes_to_icc(failures)
    test_file_url_html_routes_to_icc_without_python_binary(failures)
    test_local_html_file_routes_to_icc_without_python_binary(failures)
