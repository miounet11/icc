#!/usr/bin/env python3
"""
Regression test: `icc --version` must not scan huge sibling app lists just to
...
def resolve_icc_cli() -> str:
    ...
    candidates: list[str] = []
    candidates.extend(glob.glob(os.path.expanduser("~/Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/icc")))
    ...
    in_path = shutil.which("icc")
    if in_path:
        return in_path

    raise RuntimeError("Unable to find icc CLI binary. Set ICC_CLI_BIN.")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="icc-version-memory-guard-") as root:
        fixture_cli = build_fixture(root, cli_path)
        result = run_with_limits(fixture_cli, "--version")

    if result["failure_reason"]:
        print("FAIL: `icc --version` exceeded runtime guard")
    ...
    if result["exit_code"] != 0:
        print("FAIL: `icc --version` exited non-zero")
    ...
    print(
        "PASS: `icc --version` exits within memory/time limits "
