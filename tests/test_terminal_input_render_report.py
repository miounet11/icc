...
Environment:
  ICC_SOCKET or ICC_SOCKET_PATH can override the socket path.
"""
...
SOCKET_PATH = os.environ.get("ICC_SOCKET") or os.environ.get("ICC_SOCKET_PATH") or "/tmp/icc-debug.sock"
...
def _focused_panel_id(c: icc) -> str:
    surfaces = c.list_surfaces()
    if not surfaces:
        raise iccError("Expected at least 1 surface")
    return next((sid for _i, sid, focused in surfaces if focused), surfaces[0][1])


def _snap_panel(c: icc, panel_id: str, label: str) -> Shot:
...
def _panel_sequence_blink_and_type(c: icc, panel_id: str, prefix: str, typed_char: str = "x") -> tuple[list[Shot], dict]:
    shots: list[Shot] = []
...
<head>
  <meta charset="utf-8" />
  <title>icc terminal input render report</title>
  <style>
    :root {{
      --bg: #0b0f14;
      --panel: #111826;
...
<body>
  <h1>icc terminal input render report</h1>
