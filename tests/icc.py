...
A client library for programmatically controlling icc via Unix socket.
...
_DEFAULT_DEBUG_BUNDLE_ID = "com.icc.app.debug"
...
def _default_bundle_id() -> str:
    override = os.environ.get("ICC_BUNDLE_ID")
    if override:
        return override

    tag = os.environ.get("ICC_TAG")
    if tag:
...
def _default_socket_path() -> str:
    ...
    override = os.environ.get("ICC_SOCKET_PATH")
    if override:
    ...
    tagged = glob.glob("/tmp/icc-debug-*.sock")
    tagged.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "icc*.sock")))
...
class icc:
    """Client for controlling icc via Unix socket"""
    ...
    def _send_command(self, command: str) -> str:
        ...
        try:
            ...
            while True:
                ...
                try:
                    ...
                    if time.time() - start >= 5.0:
                        raise iccError("Command timed out")
                    continue
                if not chunk:
                    break
                data += chunk.decode()
            ...
            raise iccError(f"Socket error: {e}")
    ...
    def seed_drag_pasteboard_types(self, types: List[str]) -> None:
        ...
        if not types:
            raise iccError("seed_drag_pasteboard_types requires at least one type")
        payload = ",".join(t.strip() for t in types if t and t.strip())
        if not payload:
            raise iccError("seed_drag_pasteboard_types requires at least one non-empty type")
    ...
    def layout_debug(self) -> dict:
        ...
        try:
            ...
            raise iccError(f"layout_debug JSON decode failed: {e}: {payload[:200]}")
    ...
    def render_stats(self, panel: Union[str, int, None] = None) -> dict:
        ...
        try:
            ...
            raise iccError(f"render_stats JSON decode failed: {e}: {payload[:200]}")
    ...
    def panel_snapshot(self, panel: Union[str, int], label: str = "") -> dict:
        ...
        if len(parts) != 5:
            raise iccError(f"panel_snapshot parse failed: {response}")
...
def main():
    """CLI interface for icc"""
    import sys
    import argparse

    parser = argparse.ArgumentParser(description="icc CLI")
    ...
    try:
        with icc(args.socket) as client:
            if not args.command:
                # Interactive mode
                print("icc CLI (type 'help' for commands, 'quit' to exit)")
                while True:
                    try:
                        line = input("> ").strip()
