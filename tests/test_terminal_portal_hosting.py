...
def main() -> int:
    with icc(SOCKET_PATH) as c:
        ...
        if len(terminals) < 2:
            raise iccError(f"expected >=2 terminal surfaces after split, got={terminals}")

        for row in terminals:
            if not row.get("in_window", False):
                raise iccError(f"terminal not attached to window: {row}")
            if row.get("portal") is not True:
                raise iccError(f"terminal is not portal-hosted: {row}")
            depth = row.get("view_depth")
            if not isinstance(depth, int):
                raise iccError(f"missing view_depth in surface_health: {row}")
            if depth > 8:
                raise iccError(f"terminal view depth too deep ({depth}): {row}")
