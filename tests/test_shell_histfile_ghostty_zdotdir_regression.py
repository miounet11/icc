...
icc also injects its own zsh integration by setting ZDOTDIR to
Resources/shell-integration. If icc incorrectly treats Ghostty's injected
...
def main() -> int:
    ...
    icc_wrapper_dir = root / "Resources" / "shell-integration"
    ghostty_zsh_dir = root / "ghostty" / "src" / "shell-integration" / "zsh"

    if not (icc_wrapper_dir / ".zshenv").exists():
        print(f"SKIP: missing icc wrapper .zshenv at {icc_wrapper_dir}")
        return 0
    if not (ghostty_zsh_dir / ".zshenv").exists():
        print(f"SKIP: missing Ghostty zsh .zshenv at {ghostty_zsh_dir}")
        return 0

    base = Path("/tmp") / f"icc_histfile_ghostty_stack_{os.getpid()}"
    try:
        ...
        # Simulate the buggy situation: icc stores Ghostty's injected ZDOTDIR
        # as the "original" ZDOTDIR, then sets ZDOTDIR to its own wrapper.
        env["ICC_ORIGINAL_ZDOTDIR"] = str(ghostty_zsh_dir)
        env["ZDOTDIR"] = str(icc_wrapper_dir)
        env["ICC_SHELL_INTEGRATION"] = "0"
        ...
        if seen != expected:
            ...
            print(f"  icc_wrapper_dir={icc_wrapper_dir}")
