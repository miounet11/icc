        ...
        try:
            ...
            with conn:
                ...
                if not self.commands:
                    raise RuntimeError("icc CLI never sent a command to the teardown test socket")
...
def main() -> int:
    ...
    temp_dir = tempfile.TemporaryDirectory(prefix="icc-claude-hook-stop-")
    try:
        root = Path(temp_dir.name)
        socket_path = str(root / "icc.sock")
        ...
        env["ICC_WORKSPACE_ID"] = str(uuid.uuid4())
        env["ICC_SURFACE_ID"] = str(uuid.uuid4())
        env["ICC_CLAUDE_HOOK_STATE_PATH"] = str(state_path)
