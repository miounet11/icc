...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "iccd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  iccd-remote version")
	_, _ = fmt.Fprintln(w, "  iccd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  iccd-remote cli <command> [args...]")
}
