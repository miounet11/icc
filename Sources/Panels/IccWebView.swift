...
extension WKWebView {
    var iccIsElementFullscreenActiveOrTransitioning: Bool {
    ...
    func iccIsManagedByExternalFullscreenWindow(relativeTo expectedWindow: NSWindow?) -> Bool {
        guard iccIsElementFullscreenActiveOrTransitioning else { return false }
...
/// the first responder.
final class IccWebView: WKWebView {
    ...
    static func hasRecentMiddleClickIntent(for webView: WKWebView) -> Bool {
        guard let webView = webView as? IccWebView else { return false }
    ...
    private static func recordMiddleClickIntent(for webView: IccWebView) {
    ...
    var contextMenuLinkURLProvider: ((IccWebView, NSPoint, @escaping (URL?) -> Void) -> Void)?
    ...
    private static let blockedDragTypes: Set<NSPasteboard.PasteboardType> = [
        ...
        NSPasteboard.PasteboardType("com.icc.sidebar-tab-reorder"),
    ]
