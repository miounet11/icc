...
enum BrowserFindJavaScript {
    ...
    static func searchScript(query: String) -> String {
        ...
        return """
        (() => {
          const MARK_CLASS = '__icc-find';
          const CURRENT_CLASS = '__icc-find-current';
          ...
          window.__iccFindMatches = matches;
          window.__iccFindIndex = 0;
          ...
          if (!document.getElementById('__icc-find-style')) {
            const style = document.createElement('style');
            style.id = '__icc-find-style';
            style.textContent = `
              mark.__icc-find { background: #facc15; color: #000; border-radius: 2px; }
              mark.__icc-find.__icc-find-current { background: #f97316; color: #fff; }
            `;
    ...
    static func nextScript() -> String {
        """
        (() => {
          const matches = window.__iccFindMatches || [];
          if (matches.length === 0) return JSON.stringify({ total: 0, current: 0 });
          let idx = window.__iccFindIndex || 0;
          if (!matches[idx] || !matches[idx].isConnected) {
            window.__iccFindMatches = [];
            window.__iccFindIndex = 0;
            return JSON.stringify({ total: 0, current: 0 });
          }
          matches[idx].classList.remove('__icc-find-current');
          ...
          matches[idx].classList.add('__icc-find-current');
          matches[idx].scrollIntoView({ block: 'center', behavior: 'smooth' });
          window.__iccFindIndex = idx;
    ...
    static func clearScript() -> String {
        ...
        (() => {
          ...
          window.__iccFindMatches = [];
          window.__iccFindIndex = 0;
          const style = document.getElementById('__icc-find-style');
    ...
    document.querySelectorAll('mark.__icc-find').forEach(mark => {
