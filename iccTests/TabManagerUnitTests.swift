...
@MainActor
final class TabManagerWorkspaceCreationThreadingTests: XCTestCase {
    func testAddWorkspaceCalledOffMainThreadMarshalsMutationToMainThread() {
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            ...
            createdWorkspaceBox.withLock { $0 = workspace }
            completion.fulfill()
        }
        ...
        XCTAssertEqual(manager.selectedTabId, manager.tabs.last?.id)

        let createdWorkspace = createdWorkspaceBox.withLock { $0 }
        XCTAssertNotNil(createdWorkspace)
        XCTAssertEqual(createdWorkspace?.id, manager.tabs.last?.id)
        XCTAssertTrue(Thread.isMainThread, "Assertions should observe the post-hop main-thread state")
    }
}
...
final class TabManagerPullRequestProbeTests: XCTestCase {
    func testGitHubRepositorySlugsPrioritizeUpstreamThenOriginAndDeduplicate() {
        let output = """
        origin https://github.com/austinwang/icc.git (fetch)
        origin https://github.com/austinwang/icc.git (push)
        upstream git@github.com:manaflow-ai/icc.git (fetch)
        upstream git@github.com:manaflow-ai/icc.git (push)
        backup ssh://git@github.com/manaflow-ai/icc.git (fetch)
        mirror https://gitlab.com/manaflow-ai/icc.git (fetch)
        """

        XCTAssertEqual(
            TabManager.githubRepositorySlugs(fromGitRemoteVOutput: output),
            ["manaflow-ai/icc", "austinwang/icc"]
        )
    }
    ...
    func testPreferredPullRequestPrefersOpenOverMergedAndClosed() {
        let candidates = [
            TabManager.GitHubPullRequestProbeItem(
                number: 1889,
                state: "MERGED",
                url: "https://github.com/manaflow-ai/icc/pull/1889",
                updatedAt: "2026-03-20T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1891,
                state: "OPEN",
                url: "https://github.com/manaflow-ai/icc/pull/1891",
                updatedAt: "2026-03-19T18:00:00Z"
            ),
            TabManager.GitHubPullRequestProbeItem(
                number: 1800,
                state: "CLOSED",
                url: "https://github.com/manaflow-ai/icc/pull/1800",
    ...
    func testPreferredPullRequestPrefersMostRecentlyUpdatedWithinSameStatus() {
        let olderOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1880,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/icc/pull/1880",
            updatedAt: "2026-03-18T18:00:00Z"
        )
        let newerOpen = TabManager.GitHubPullRequestProbeItem(
            number: 1890,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/icc/pull/1890",
            updatedAt: "2026-03-20T18:00:00Z"
        )
    ...
    func testPreferredPullRequestIgnoresMalformedCandidates() {
        let valid = TabManager.GitHubPullRequestProbeItem(
            number: 1888,
            state: "OPEN",
            url: "https://github.com/manaflow-ai/icc/pull/1888",
            updatedAt: "2026-03-20T18:00:00Z"
        )
        ...
        XCTAssertEqual(
            TabManager.preferredPullRequest(from: [
                TabManager.GitHubPullRequestProbeItem(
                    number: 9999,
                    state: "WHATEVER",
                    url: "https://github.com/manaflow-ai/icc/pull/9999",
                    updatedAt: "2026-03-21T18:00:00Z"
                ),
                TabManager.GitHubPullRequestProbeItem(
                    number: 10000,
