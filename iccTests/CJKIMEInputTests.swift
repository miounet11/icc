...
private extension GhosttyNSView {
    @objc func iccUnitTest_interpretKeyEvents(_ eventArray: [NSEvent]) {
        if let hook = cjkIMEInterpretKeyEventsHook, hook(self, eventArray) {
            return
        }
        iccUnitTest_interpretKeyEvents(eventArray)
    }
}
...
private func installCJKIMEInterpretKeyEventsSwizzle() {
    ...
    let swizzledSelector = #selector(GhosttyNSView.iccUnitTest_interpretKeyEvents(_:))
...
// MARK: - Space release regression (Codex hold-to-talk in icc)
