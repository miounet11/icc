...
final class FinderServicePathResolverTests: XCTestCase {
    func testOrderedUniqueDirectoriesUsesParentForFilesAndDedupes() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/icc-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/icc-services/project/README.md", isDirectory: false),
            URL(fileURLWithPath: "/tmp/icc-services/../icc-services/project", isDirectory: true),
            URL(fileURLWithPath: "/tmp/icc-services/other", isDirectory: true),
        ]
        ...
        XCTAssertEqual(
            directories,
            [
                "/tmp/icc-services/project",
                "/tmp/icc-services/other",
            ]
        )
    }
    ...
    func testOrderedUniqueDirectoriesPreservesFirstSeenOrder() {
        let input: [URL] = [
            URL(fileURLWithPath: "/tmp/icc-services/b", isDirectory: true),
            URL(fileURLWithPath: "/tmp/icc-services/a/file.txt", isDirectory: false),
            URL(fileURLWithPath: "/tmp/icc-services/a", isDirectory: true),
            URL(fileURLWithPath: "/tmp/icc-services/b/file.txt", isDirectory: false),
        ]
...
final class VSCodeServeWebURLBuilderTests: XCTestCase {
    ...
    func testOpenFolderURLAppendsFolderQueryWhilePreservingToken() {
        ...
        let url = VSCodeServeWebURLBuilder.openFolderURL(
            ...
            directoryPath: "/Users/tester/Projects/icc"
        )

        let components = URLComponents(url: url!, resolvingAgainstBaseURL: false)
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "tkn" })?.value, "test-token")
        XCTAssertEqual(components?.queryItems?.first(where: { $0.name == "folder" })?.value, "/Users/tester/Projects/icc")
    }
