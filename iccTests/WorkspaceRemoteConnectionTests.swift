...
final class WorkspaceRemoteConnectionTests: XCTestCase {
    ...
    private func runRelayZshHistfile(
        ...
        let home = fileManager.temporaryDirectory.appendingPathComponent("icc-relay-zsh-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".icc/relay/64011.shell")
    ...
    func testRemoteRelayMetadataCleanupScriptRemovesMatchingSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("icc-relay-cleanup-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".icc/relay")
        let socketAddrURL = home.appendingPathComponent(".icc/socket_addr")
    ...
    func testRemoteRelayMetadataCleanupScriptPreservesDifferentSocketAddr() {
        let fileManager = FileManager.default
        let home = fileManager.temporaryDirectory.appendingPathComponent("icc-relay-cleanup-preserve-\(UUID().uuidString)")
        let relayDir = home.appendingPathComponent(".icc/relay")
        let socketAddrURL = home.appendingPathComponent(".icc/socket_addr")
    ...
    func testRemoteTerminalSurfaceLookupTracksOnlyActiveSSHSurfaces() throws {
        ...
        let config = WorkspaceRemoteConfiguration(
            ...
            localSocketPath: "/tmp/icc-debug-test.sock",
    ...
    func testRemoteDropPathUsesLowercasedExtensionAndProvidedUUID() throws {
        ...
        XCTAssertEqual(remotePath, "/tmp/icc-drop-12345678-1234-1234-1234-1234567890ab.png")
    }
    ...
    func testDetectedSSHUploadFailureCleansUpEarlierRemoteUploads() throws {
        ...
        let directoryURL = fileManager.temporaryDirectory.appendingPathComponent(
            "icc-detected-ssh-upload-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    func testDetectsForegroundSSHSessionWithShortControlPathFlag() {
        ...
        XCTAssertEqual(session?.controlPath, "/tmp/icc-ssh-%C")
        let scpArgs = session?.scpArgumentsForTesting(
            localPath: "/tmp/local.png",
            remotePath: "/tmp/icc-drop-123.png"
        ) ?? []
        XCTAssertTrue(scpArgs.contains("ControlPath=/tmp/icc-ssh-%C"))
    ...
    func testDetectedSSHSessionBracketsIPv6LiteralSCPDestination() {
        ...
        XCTAssertEqual(scpArgs.last, "lawrence@[2001:db8::1]:/tmp/icc-drop-123.png")
    }
    ...
    func testProxyOnlyErrorsKeepSSHWorkspaceConnectedAndLoggedInSidebar() {
        ...
        let proxyError = "Remote proxy to icc-macmini unavailable: Failed to start local daemon proxy: daemon RPC timeout waiting for hello response (retry in 3s)"
        workspace.applyRemoteConnectionStateUpdate(.error, detail: proxyError, target: "icc-macmini")
        ...
        XCTAssertEqual(
            ...
            "Remote proxy unavailable (icc-macmini): \(proxyError)"
        )
        ...
        workspace.applyRemoteConnectionStateUpdate(.connecting, detail: "Connecting to icc-macmini", target: "icc-macmini")
        ...
        workspace.applyRemoteConnectionStateUpdate(
            .connected,
            detail: "Connected to icc-macmini via shared local proxy 127.0.0.1:9999",
            target: "icc-macmini"
        )
...
final class CLINotifyProcessIntegrationTests: XCTestCase {
    ...
    private func bundledCLIPath() throws -> String {
        ...
        while let item = enumerator?.nextObject() as? URL {
            guard item.lastPathComponent == "icc",
                  item.path.contains(".app/Contents/Resources/bin/icc") else {
                continue
            }
            return item.path
        }

        throw XCTSkip("Bundled icc CLI not found in \(appBundleURL.path)")
    }
    ...
    func testNotifyFallsBackFromStaleCallerWorkspaceAndSurfaceIDs() throws {
        ...
        environment["ICC_SOCKET_PATH"] = socketPath
        environment["ICC_WORKSPACE_ID"] = staleWorkspace
        environment["ICC_SURFACE_ID"] = staleSurface
        environment["ICC_CLI_SENTRY_DISABLED"] = "1"
        environment["ICC_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyPrefersCallerTTYOverFocusedSurfaceWhenCallerIDsAreStale() throws {
        ...
        environment["ICC_WORKSPACE_ID"] = staleWorkspace
        environment["ICC_SURFACE_ID"] = staleSurface
        environment["ICC_CLI_TTY_NAME"] = callerTTY
        environment["ICC_CLI_SENTRY_DISABLED"] = "1"
        environment["ICC_CLAUDE_HOOK_SENTRY_DISABLED"] = "1"
    ...
    func testNotifyInTmuxPrefersCallerTTYOverStaleValidSurfaceID() throws {
        ...
        var environment = ProcessInfo.processInfo.environment
        environment["ICC_SOCKET_PATH"] = socketPath
        environment["ICC_WORKSPACE_ID"] = workspaceId
        environment["ICC_SURFACE_ID"] = staleSurface
        environment["ICC_CLI_TTY_NAME"] = callerTTY
