...
final class GhosttyPasteboardHelperTests: XCTestCase {
    ...
    func testHTMLOnlyPasteboardExtractsPlainText() {
        let pasteboard = NSPasteboard(name: .init("icc-test-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setString("<p>Hello <strong>world</strong></p>", forType: .html)

        XCTAssertEqual(iccPasteboardStringContentsForTesting(pasteboard), "Hello world")
        XCTAssertNil(iccPasteboardImagePathForTesting(pasteboard))
    }

    func testImageHTMLClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-image-html-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertNil(iccPasteboardStringContentsForTesting(pasteboard))

        let imagePath = try XCTUnwrap(iccPasteboardImagePathForTesting(pasteboard))
    ...
    func testImageHTMLClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-image-html-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        XCTAssertEqual(iccPasteboardStringContentsForTesting(pasteboard), "Hello")
        XCTAssertNil(iccPasteboardImagePathForTesting(pasteboard))
    }

    func testJPEGClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-jpeg-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDClipboardFallsBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-rtfd-attachment-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testAttachmentOnlyRTFDNonImageClipboardDoesNotFallBackToImagePath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-rtfd-non-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRTFDClipboardWithVisibleTextPrefersText() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-rtfd-text-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testImageOnlyPasteboardProducesTempFileURL() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-drop-image-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .red), forType: .png)

        let fileURL = try XCTUnwrap(iccPasteboardImageFileURLForTesting(pasteboard))
    ...
    func testRemoteImageDropPlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLocalImageDropPlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-local-drop-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePastePlanUploadsMaterializedFile() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteFileURLPastePlanUploadsReadableFile() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-file-url-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteDirectoryPastePlanFallsBackToEscapedPathInsertion() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-directory-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanSkipsTargetResolutionForPlainText() {
        let pasteboard = NSPasteboard(name: .init("icc-test-lazy-text-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testLazyPastePlanResolvesTargetForFileURLPaste() throws {
        ...
        let pasteboard = NSPasteboard(name: .init("icc-test-lazy-file-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
        XCTAssertTrue(pasteboard.writeObjects([fileURL as NSURL]))
    ...
    func testLocalImagePastePlanInsertsEscapedLocalPath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-local-paste-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImagePasteExecutionUploadsAndCompletesWithRemotePath() throws {
        ...
        TerminalImageTransferPlanner.executeForTesting(
            ...
            uploadWorkspaceRemote: { _, _, finish in finish(.success(["/tmp/icc-drop-123.png"])) },
        ...
        XCTAssertEqual(completedText, "/tmp/icc-drop-123.png")
    }
    ...
    func testCancelledRemoteImagePasteExecutionSuppressesCompletionHandlers() throws {
        ...
        completion?(.success(["/tmp/icc-drop-cancelled.png"]))
    ...
    func testCancelledOperationSuppressesLateLocalInsert() {
        ...
        let returnedOperation = TerminalImageTransferPlanner.executeForTesting(
            plan: .insertText("/tmp/icc-drop-local.png"),
            operation: operation,
    ...
    func testRemoteImageDropHandlerUploadsAndSendsRemotePath() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-handler-\(UUID().uuidString)"))
        pasteboard.clearContents()
        ...
        let handled = GhosttyNSView.handleDropForTesting(
            ...
            uploadRemote: { urls, finish in
                uploadedURLs = urls
                finish(.success(["/tmp/icc-drop-abc123.png"]))
            },
        ...
        XCTAssertEqual(sentText, ["/tmp/icc-drop-abc123.png"])
        XCTAssertEqual(failureCount, 0)
    }

    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterSuccess() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-handler-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .orange), forType: .png)
    ...
    func testRemoteDropUploadFailureTriggersFailureHandler() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-handler-fail-\(UUID().uuidString)"))
        pasteboard.clearContents()
    ...
    func testRemoteImageDropHandlerCleansUpMaterializedTemporaryImageAfterFailure() throws {
        let pasteboard = NSPasteboard(name: .init("icc-test-remote-handler-failure-cleanup-\(UUID().uuidString)"))
        pasteboard.clearContents()
        pasteboard.setData(try make1x1PNG(color: .cyan), forType: .png)
...
final class GhosttyResponderResolutionTests: XCTestCase {
    ...
    func testResolvesGhosttyViewFromDescendantResponder() {
        ...
        XCTAssertTrue(iccOwningGhosttyView(for: descendant) === ghosttyView)
    }

    func testResolvesGhosttyViewFromGhosttyResponder() {
        let ghosttyView = GhosttyNSView(frame: NSRect(x: 0, y: 0, width: 200, height: 120))
        XCTAssertTrue(iccOwningGhosttyView(for: ghosttyView) === ghosttyView)
    }

    func testReturnsNilForUnrelatedResponder() {
        let view = FocusProbeView(frame: NSRect(x: 0, y: 0, width: 40, height: 40))
        XCTAssertNil(iccOwningGhosttyView(for: view))
    }
}
...
final class TerminalOpenURLTargetResolutionTests: XCTestCase {
    ...
    func testResolvesHostlessHTTPSAsExternal() throws {
        let target = try XCTUnwrap(resolveTerminalOpenURLTarget("https:///tmp/icc.txt"))
        switch target {
        case let .external(url):
            XCTAssertEqual(url.scheme, "https")
            XCTAssertNil(url.host)
            XCTAssertEqual(url.path, "/tmp/icc.txt")
        default:
...
final class TerminalControllerSocketListenerHealthTests: XCTestCase {
    ...
    func testNonStableSocketBindFailureDoesNotFallback() {
        XCTAssertNil(
            TerminalController.fallbackSocketPathAfterBindFailure(
                requestedPath: "/tmp/icc-debug.sock",
                stage: "bind",
                errnoCode: EACCES,
                currentUserID: 501
            )
    ...
    private func makeTempSocketPath() -> String {
        "/tmp/icc-socket-health-\(UUID().uuidString).sock"
    }
