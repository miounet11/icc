...
var iccUnitTestInspectorAssociationKey: UInt8 = 0
var iccUnitTestInspectorOverrideInstalled = false

extension IccWebView {
    @objc func iccUnitTestInspector() -> NSObject? {
        objc_getAssociatedObject(self, &iccUnitTestInspectorAssociationKey) as? NSObject
    }
}

extension WKWebView {
    func iccSetUnitTestInspector(_ inspector: NSObject?) {
        objc_setAssociatedObject(
            self,
            &iccUnitTestInspectorAssociationKey,
            inspector,
...
func installIccUnitTestInspectorOverride() {
    guard !iccUnitTestInspectorOverrideInstalled else { return }

    guard let replacementMethod = class_getInstanceMethod(
        IccWebView.self,
        #selector(IccWebView.iccUnitTestInspector)
    ) else {
    ...
    guard added else {
        fatalError("Unable to install IccWebView _inspector test override")
    }

    iccUnitTestInspectorOverrideInstalled = true
}

final class IccWebViewKeyEquivalentTests: XCTestCase {
    ...
    func testCmdNRoutesToMainMenuWhenWebViewIsFirstResponder() {
        ...
        let webView = IccWebView(frame: .zero, configuration: WKWebViewConfiguration())
    ...
    func testCanBlockFirstResponderAcquisitionWhenPaneIsUnfocused() {
        ...
        let webView = IccWebView(frame: container.bounds, configuration: WKWebViewConfiguration())
    ...
    func testWindowFirstResponderGuardAllowsPointerInitiatedClickFocusFromPortalHostedInspectorSibling() {
        ...
        let webView = IccWebView(frame: slot.bounds, configuration: WKWebViewConfiguration())
        webView.autoresizingMask = [.width, .height]
    ...
    func testCmdBacktickDoesNotRouteDirectlyToMainMenuWhenWebViewIsFirstResponder() {
        ...
        XCTAssertFalse(
            spy.invoked,
            "IccWebView should not route Cmd+` directly to the menu when WebKit is first responder"
        )
    }
...
@MainActor
final class IccWebViewContextMenuTests: XCTestCase {
    ...
    func testWillOpenMenuAddsOpenLinkInDefaultBrowserAndRoutesSelectionToDefaultBrowserOpener() {
        _ = NSApplication.shared
        let webView = IccWebView(frame: NSRect(x: 0, y: 0, width: 800, height: 600), configuration: WKWebViewConfiguration())
        let menu = NSMenu()
...
final class BrowserSessionHistoryRestoreTests: XCTestCase {
    ...
    func testGoBackPrefersLiveWKWebViewHistoryBeforeRestoredFallback() throws {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-browser-history-\(UUID().uuidString)", isDirectory: true)
...
final class BrowserDeveloperToolsVisibilityPersistenceTests: XCTestCase {
    ...
    override class func setUp() {
        super.setUp()
        installIccUnitTestInspectorOverride()
    }
    ...
    private func makePanelWithInspector(
        ...
        panel.webView.iccSetUnitTestInspector(inspector)
...
@MainActor
final class IccWebViewDragRoutingTests: XCTestCase {
    func testRejectsInternalPaneDragEvenWhenFilePromiseTypesArePresent() {
        XCTAssertTrue(
            IccWebView.shouldRejectInternalPaneDrag([
    ...
    func testAllowsRegularExternalFileDrops() {
        XCTAssertFalse(IccWebView.shouldRejectInternalPaneDrag([.fileURL]))
    }
}
...
final class BrowserLinkOpenSettingsTests: XCTestCase {
    ...
    func testTerminalLinksDefaultToIccBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInIccBrowser(defaults: defaults))
    }

    func testTerminalLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInIccBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openTerminalLinksInIccBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInIccBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openTerminalLinksInIccBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksDefaultToIccBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInIccBrowser(defaults: defaults))
    }

    func testSidebarPullRequestLinksPreferenceUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInIccBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.openSidebarPullRequestLinksInIccBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openSidebarPullRequestLinksInIccBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.openSidebarPullRequestLinksInIccBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionDefaultsToIccBrowser() {
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInIccBrowser(defaults: defaults))
    }

    func testOpenCommandInterceptionUsesStoredValue() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInIccBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.interceptTerminalOpenCommandInIccBrowser(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.interceptTerminalOpenCommandInIccBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.interceptTerminalOpenCommandInIccBrowser(defaults: defaults))
    }
    ...
    func testSettingsInitialOpenCommandInterceptionValueFallsBackToLegacyLinkToggleWhenUnset() {
        defaults.set(false, forKey: BrowserLinkOpenSettings.openTerminalLinksInIccBrowserKey)
        XCTAssertFalse(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInIccBrowserValue(defaults: defaults))

        defaults.set(true, forKey: BrowserLinkOpenSettings.openTerminalLinksInIccBrowserKey)
        XCTAssertTrue(BrowserLinkOpenSettings.initialInterceptTerminalOpenCommandInIccBrowserValue(defaults: defaults))
    }
...
final class BrowserNavigableURLResolutionTests: XCTestCase {
    func testResolvesFileSchemeAsNavigableURL() throws {
        let resolved = try XCTUnwrap(resolveBrowserNavigableURL("file:///tmp/icc-local-test.html"))
        XCTAssertTrue(resolved.isFileURL)
        XCTAssertEqual(resolved.path, "/tmp/icc-local-test.html")
    }
...
final class BrowserExternalNavigationSchemeTests: XCTestCase {
    ...
    func testEmbeddedBrowserSchemesStayInWebView() throws {
        ...
        let file = try XCTUnwrap(URL(string: "file:///tmp/icc-local-test.html"))
