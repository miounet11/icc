...
final class GhosttyConfigTests: XCTestCase {
    ...
    func testThemeSearchPathsIncludeXDGDataDirsThemes() {
        let pathA = "/tmp/icc-theme-a"
        let pathB = "/tmp/icc-theme-b"
    ...
    func testLoadThemeResolvesPairedThemeValueByColorScheme() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ghostty-theme-pair-\(UUID().uuidString)")
    ...
    func testLoadThemeResolvesBuiltinAliasFromGhosttyResourcesDir() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ghostty-themes-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedLightAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ghostty-solarized-light-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testLoadThemeResolvesITerm2SolarizedDarkAliasToLegacyThemeName() throws {
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-ghostty-solarized-dark-\(UUID().uuidString)")
        let themesDir = root.appendingPathComponent("themes")
        try FileManager.default.createDirectory(at: themesDir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: root) }
    ...
    func testIccAppSupportConfigURLsUseReleaseConfigForDebugBundleWithoutCurrentConfig() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            let releaseConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.icc.app",
                filename: "config",
                contents: "font-size = 13\n"
            )

            XCTAssertEqual(
                GhosttyApp.iccAppSupportConfigURLs(
                    currentBundleIdentifier: "com.icc.app.debug",
                    appSupportDirectory: appSupportDirectory
                ),
                [releaseConfigURL]
            )
    ...
    func testIccAppSupportConfigURLsPreferCurrentBundleConfigWhenPresent() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            ...
            let currentConfigURL = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.icc.app.debug.issue-829",
            ...
            XCTAssertEqual(
                GhosttyApp.iccAppSupportConfigURLs(
                    currentBundleIdentifier: "com.icc.app.debug.issue-829",
                    appSupportDirectory: appSupportDirectory
                ),
                [currentConfigURL]
            )
    ...
    func testIccAppSupportConfigURLsSkipReleaseFallbackForNonDebugBundle() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.icc.app",
    ...
    func testIccAppSupportConfigURLsIgnoreMissingOrEmptyFiles() throws {
        try withTemporaryAppSupportDirectory { appSupportDirectory in
            _ = try writeAppSupportConfig(
                appSupportDirectory: appSupportDirectory,
                bundleIdentifier: "com.icc.app",
    ...
    func testClaudeCodeIntegrationDefaultsToEnabledWhenUnset() {
        let suiteName = "icc.tests.claude-hooks.\(UUID().uuidString)"
    ...
    func testTelemetryDefaultsToEnabledWhenUnset() {
        let suiteName = "icc.tests.telemetry.\(UUID().uuidString)"
        guard let defaults = UserDefaults(suiteName: suiteName) else {
            XCTFail("Failed to create isolated user defaults suite")
            return
        }
    ...
    private func withTemporaryAppSupportDirectory(
        ...
        let directory = fileManager.temporaryDirectory
            .appendingPathComponent("icc-app-support-\(UUID().uuidString)", isDirectory: true)
...
final class WindowTransparencyDecisionTests: XCTestCase {
    ...
    func testTranslucentOpacityForcesClearWindowBackgroundOutsideSidebarBlendModePath() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertFalse(iccShouldUseTransparentBackgroundWindow())
            XCTAssertTrue(iccShouldUseClearWindowBackground(for: 0.80))
            XCTAssertFalse(iccShouldUseClearWindowBackground(for: 1.0))
        }
    }
    ...
    func testBehindWindowGlassPathStillControlsTransparentWindowFallback() {
        withTemporaryWindowBackgroundDefaults {
            ...
            XCTAssertEqual(iccShouldUseTransparentBackgroundWindow(), expectedTransparentFallback)
            XCTAssertEqual(
                iccShouldUseClearWindowBackground(for: 1.0),
...
final class WorkspaceRemoteDaemonManifestTests: XCTestCase {
    func testParsesEmbeddedRemoteDaemonManifestJSON() throws {
        let manifestJSON = """
        {
          ...
          "appVersion": "0.62.0",
          "releaseTag": "v0.62.0",
          "releaseURL": "https://github.com/manaflow-ai/icc/releases/tag/v0.62.0",
          "checksumsAssetName": "iccd-remote-checksums.txt",
          "checksumsURL": "https://github.com/manaflow-ai/icc/releases/download/v0.62.0/iccd-remote-checksums.txt",
          "entries": [
            {
              "goOS": "linux",
              "goArch": "amd64",
              "assetName": "iccd-remote-linux-amd64",
              "downloadURL": "https://github.com/manaflow-ai/icc/releases/download/v0.62.0/iccd-remote-linux-amd64",
              "sha256": "abc123"
            }
          ]
        }
        ...
        XCTAssertEqual(manifest?.entry(goOS: "linux", goArch: "amd64")?.assetName, "iccd-remote-linux-amd64")
    }
    ...
    func testRemoteDaemonCachePathIsVersionedByPlatform() throws {
        ...
        XCTAssertTrue(url.path.contains("/Application Support/icc/remote-daemons/0.62.0/linux-arm64/"))
        XCTAssertEqual(url.lastPathComponent, "iccd-remote")
    }
}
...
final class RemoteLoopbackHTTPRequestRewriterTests: XCTestCase {
    func testRewritesLoopbackAliasHostHeadersToLocalhost() {
        let original = Data(
            (
                "GET /demo HTTP/1.1\r\n" +
                "Host: icc-loopback.localtest.me:3000\r\n" +
                "Origin: http://icc-loopback.localtest.me:3000\r\n" +
                "Referer: http://icc-loopback.localtest.me:3000/app\r\n" +
                "\r\n"
            ).utf8
        )
        ...
        let rewritten = RemoteLoopbackHTTPRequestRewriter.rewriteIfNeeded(
            data: original,
            aliasHost: "icc-loopback.localtest.me"
        )
        ...
        XCTAssertFalse(text.contains("icc-loopback.localtest.me"))
    }

    func testRewritesAbsoluteFormRequestLineForLoopbackAlias() {
        let original = Data(
            (
                "GET http://icc-loopback.localtest.me:3000/demo HTTP/1.1\r\n" +
                "Host: icc-loopback.localtest.me:3000\r\n" +
                "\r\n"
            ).utf8
        )
    ...
    func testRewritesLoopbackResponseHeadersBackToAlias() {
        ...
        let text = String(decoding: rewritten, as: UTF8.self)
        XCTAssertTrue(text.contains("Location: http://icc-loopback.localtest.me:3000/login"))
        XCTAssertTrue(text.contains("Access-Control-Allow-Origin: http://icc-loopback.localtest.me:3000"))
        XCTAssertTrue(text.contains("Set-Cookie: sid=1; Domain=icc-loopback.localtest.me; Path=/"))
    }
}

final class GhosttyTerminalStartupEnvironmentTests: XCTestCase {
    func testMergedStartupEnvironmentAllowsSessionReplayAndInitialEnvICCKeys() {
        let replayPath = "/tmp/icc-replay-\(UUID().uuidString)"
        let merged = TerminalSurface.mergedStartupEnvironment(
            base: [
                "PATH": "/usr/bin",
                "ICC_SURFACE_ID": "managed-surface"
            ],
            protectedKeys: ["PATH", "ICC_SURFACE_ID"],
            additionalEnvironment: [
                SessionScrollbackReplayStore.environmentKey: replayPath
            ],
            initialEnvironmentOverrides: [
                "ICC_INITIAL_ENV_TOKEN": "token-123"
            ]
        )

        XCTAssertEqual(merged[SessionScrollbackReplayStore.environmentKey], replayPath)
        XCTAssertEqual(merged["ICC_INITIAL_ENV_TOKEN"], "token-123")
    }
    ...
    func testMergedStartupEnvironmentProtectsManagedKeysOnly() {
        let merged = TerminalSurface.mergedStartupEnvironment(
            ...
            protectedKeys: ["PATH", "ICC_SURFACE_ID"],
            additionalEnvironment: [
                "ICC_SURFACE_ID": "user-surface",
                "CUSTOM_FLAG": "1"
            ],
            initialEnvironmentOverrides: [
                "PATH": "/tmp/bin",
                "ICC_SURFACE_ID": "override-surface"
            ]
        )

        XCTAssertEqual(merged["PATH"], "/usr/bin")
        XCTAssertEqual(merged["ICC_SURFACE_ID"], "managed-surface")
...
final class BrowserPanelRemoteStoreTests: XCTestCase {
    ...
    func testRemoteWorkspaceDefersInitialNavigationUntilProxyEndpointIsReady() {
        ...
        XCTAssertEqual(panel.preferredURLStringForOmnibar(), url.absoluteString)
        XCTAssertEqual(panel.webView.url?.host, "icc-loopback.localtest.me")
    }
    ...
    func testBrowserMoveIntoRemoteWorkspaceRebuildsWebsiteDataStoreScope() throws {
        ...
        destination.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/icc-store-dest.sock",
    ...
    func testBrowserMoveOutOfRemoteWorkspaceRestoresDefaultWebsiteDataStore() throws {
        ...
        source.configureRemoteConnection(
            WorkspaceRemoteConfiguration(
                ...
                localSocketPath: "/tmp/icc-store-source.sock",
                terminalStartupCommand: "ssh icc-macmini"
            ),
            autoConnect: false
        )
    ...
    func testProxyBrokerTransportKeyIgnoresControlPath() {
        let first = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/icc-ssh-501-64000-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64000,
            relayID: "relay-a",
            relayToken: "token-a",
            localSocketPath: "/tmp/icc-a.sock",
            terminalStartupCommand: "ssh icc-macmini"
        )
        let second = WorkspaceRemoteConfiguration(
            ...
            sshOptions: [
                "Compression=yes",
                "ControlMaster=auto",
                "ControlPath=/tmp/icc-ssh-501-64001-%C",
            ],
            localProxyPort: 9000,
            relayPort: 64001,
            relayID: "relay-b",
            relayToken: "token-b",
            localSocketPath: "/tmp/icc-b.sock",
            terminalStartupCommand: "ssh icc-macmini"
        )
...
final class SocketControlSettingsTests: XCTestCase {
    func testMigrateModeSupportsExpandedSocketModes() {
        ...
        XCTAssertEqual(SocketControlSettings.migrateMode("iccOnly"), .iccOnly)
    ...
    func testSocketModePermissions() {
        ...
        XCTAssertEqual(SocketControlMode.iccOnly.socketFilePermissions, 0o600)
    ...
    func testInvalidEnvSocketModeDoesNotOverrideUserMode() {
        XCTAssertNil(
            SocketControlSettings.envOverrideMode(
                environment: ["ICC_SOCKET_MODE": "definitely-not-a-mode"]
            )
        )
        XCTAssertEqual(
    ...
    func testStableReleaseIgnoresAmbientSocketOverrideByDefault() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ICC_SOCKET_PATH": "/tmp/icc-debug-issue-153-tmux-compat.sock",
            ],
            bundleIdentifier: "com.icc.app",
            isDebugBuild: false,
    ...
    func testNightlyReleaseUsesDedicatedDefaultAndIgnoresAmbientSocketOverride() {
        let path = SocketControlSettings.socketPath(
            ...
            bundleIdentifier: "com.icc.app.nightly",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        )

        XCTAssertEqual(path, "/tmp/icc-nightly.sock")
    }

    func testDebugBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ICC_SOCKET_PATH": "/tmp/icc-debug-my-tag.sock",
            ],
            bundleIdentifier: "com.icc.app.debug.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/icc-debug-my-tag.sock")
    }

    func testStagingBundleHonorsSocketOverrideWithoutOptInFlag() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ICC_SOCKET_PATH": "/tmp/icc-staging-my-tag.sock",
            ],
            bundleIdentifier: "com.icc.app.staging.my-tag",
            isDebugBuild: false
        )

        XCTAssertEqual(path, "/tmp/icc-staging-my-tag.sock")
    }

    func testStableReleaseCanOptInToSocketOverride() {
        let path = SocketControlSettings.socketPath(
            environment: [
                "ICC_SOCKET_PATH": "/tmp/icc-debug-forced.sock",
                "ICC_ALLOW_SOCKET_OVERRIDE": "1",
            ],
            bundleIdentifier: "com.icc.app",
            isDebugBuild: false,
            probeStableDefaultPathEntry: { _ in .missing }
        ...
        XCTAssertEqual(path, "/tmp/icc-debug-forced.sock")
    }
    ...
    func testDefaultSocketPathByChannel() {
        ...
        XCTAssertEqual(
            ...
            "/tmp/icc-nightly.sock"
        )
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.icc.app.debug.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/icc-debug.sock"
        ...
        XCTAssertEqual(
            SocketControlSettings.defaultSocketPath(
                bundleIdentifier: "com.icc.app.staging.tag",
                isDebugBuild: false,
                probeStableDefaultPathEntry: { _ in .missing }
            ),
            "/tmp/icc-staging.sock"
        )
    }
    ...
    func testUntaggedDebugBundleBlockedWithoutLaunchTag() {
        XCTAssertTrue(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.icc.app.debug",
                isDebugBuild: true
            )
        )
    }
    ...
    func testTaggedDebugBundleAllowedWithoutLaunchTag() {
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: [:],
                bundleIdentifier: "com.icc.app.debug.tests-v1",
                isDebugBuild: true
            )
        )
    }
    ...
    func testXCUITestLaunchEnvironmentIgnoresLaunchTagGate() {
        ...
        // The app receives ICC_UI_TEST_* vars via XCUIApplication.launchEnvironment.
        XCTAssertFalse(
            SocketControlSettings.shouldBlockUntaggedDebugLaunch(
                environment: ["ICC_UI_TEST_MODE": "1"],
                bundleIdentifier: "com.icc.app.debug",
                isDebugBuild: true
            )
        )
...
final class UITestLaunchManifestTests: XCTestCase {
    func testManifestPathReadsArgumentValue() {
        XCTAssertEqual(
            UITestLaunchManifest.manifestPath(
                from: ["icc", "-iccUITestLaunchManifest", "/tmp/icc-ui-test-launch.json"]
            ),
            "/tmp/icc-ui-test-launch.json"
        )
    }
    ...
    func testManifestPathReturnsNilWithoutValue() {
        XCTAssertNil(
            UITestLaunchManifest.manifestPath(
                from: ["icc", "-iccUITestLaunchManifest"]
            )
        )
    }
    ...
    func testApplyIfPresentDecodesEnvironmentPayload() {
        let payload = """
        {"environment":{"ICC_TAG":"ui-tests-display","ICC_SOCKET_PATH":"/tmp/icc-ui-tests.sock"}}
        """.data(using: .utf8)!
        var applied: [String: String] = [:]

        UITestLaunchManifest.applyIfPresent(
            arguments: ["icc", UITestLaunchManifest.argumentName, "/tmp/icc-ui-test-launch.json"],
        ...
        XCTAssertEqual(applied["ICC_TAG"], "ui-tests-display")
        XCTAssertEqual(applied["ICC_SOCKET_PATH"], "/tmp/icc-ui-tests.sock")
    }
}
...
final class PostHogAnalyticsPropertiesTests: XCTestCase {
    ...
    func testSuperPropertiesIncludePlatformVersionAndBuild() {
        ...
        XCTAssertEqual(properties["platform"] as? String, "icc")
        XCTAssertEqual(properties["app_version"] as? String, "0.31.0")
        XCTAssertEqual(properties["app_build"] as? String, "230")
    }
    ...
    func testPropertiesOmitVersionFieldsWhenUnavailable() {
        ...
        XCTAssertEqual(superProperties["platform"] as? String, "icc")
    ...
    func testFlushPolicyIncludesDailyAndHourlyActiveEvents() {
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "icc_daily_active"))
        XCTAssertTrue(PostHogAnalytics.shouldFlushAfterCapture(event: "icc_hourly_active"))
        XCTAssertFalse(PostHogAnalytics.shouldFlushAfterCapture(event: "icc_other_event"))
    }
}
...
final class GhosttyMouseFocusTests: XCTestCase {
    ...
    private func withTempConfig(
        ...
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-test-cjk-\(UUID().uuidString)")
    ...
    func testUserConfigContainsCJKCodepointMapReturnsFalseForMissingFiles() {
        let path = NSTemporaryDirectory() + "icc-nonexistent-\(UUID().uuidString)/config"
        XCTAssertFalse(
    ...
    func testUserConfigContainsCJKCodepointMapFollowsConfigFileIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-test-cjk-include-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapFollowsRelativeIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-test-cjk-rel-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesOptionalInclude() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-test-cjk-opt-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
    ...
    func testUserConfigContainsCJKCodepointMapHandlesCyclicIncludes() throws {
        let dir = FileManager.default.temporaryDirectory
            .appendingPathComponent("icc-test-cjk-cycle-\(UUID().uuidString)")
        try FileManager.default.createDirectory(at: dir, withIntermediateDirectories: true)
        defer { try? FileManager.default.removeItem(at: dir) }
...
final class ZshShellIntegrationHandoffTests: XCTestCase {
    func testGhosttyPromptHooksLoadWhenIccRequestsZshIntegration() throws {
        let output = try runInteractiveZsh(iccLoadGhosttyIntegration: true)
    ...
    func testGhosttyPromptHooksDoNotLoadWithoutIccHandoffFlag() throws {
        let output = try runInteractiveZsh(iccLoadGhosttyIntegration: false)
    ...
    func testGhosttySemanticPatchRetriesAfterDeferredInitCreatesLiveHooks() throws {
        let output = try runInteractiveZsh(
            iccLoadGhosttyIntegration: true,
            iccLoadShellIntegration: true,
            command: """
    ...
    func testShellIntegrationWinchGuardDoesNotPrintSpacerLineOnResize() throws {
        let output = try runInteractiveZsh(
            iccLoadGhosttyIntegration: false,
            iccLoadShellIntegration: true,
            command: """
            print -r -- BEFORE
            TRAPWINCH
    ...
    func testShellIntegrationPublishesOnlyWorkspaceScopedIccEnvironmentToTmuxServerAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("icc-zsh-tmux-publish-\(UUID().uuidString)")
        ...
        _ = try runInteractiveZsh(
            iccLoadGhosttyIntegration: false,
            iccLoadShellIntegration: true,
            command: "_icc_preexec tmux; print -r -- READY",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "ICC_SOCKET_PATH": "/tmp/icc-current.sock",
                "ICC_TAG": "feat-tmux-notification-attention-state",
                "ICC_WORKSPACE_ID": "11111111-1111-1111-1111-111111111111",
                "ICC_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "ICC_TAB_ID": "11111111-1111-1111-1111-111111111111",
                "ICC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )

        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -g ICC_TAG feat-tmux-notification-attention-state"), log)
        XCTAssertTrue(log.contains("set-environment -g ICC_SOCKET_PATH /tmp/icc-current.sock"), log)
        XCTAssertTrue(log.contains("set-environment -g ICC_WORKSPACE_ID 11111111-1111-1111-1111-111111111111"), log)
        XCTAssertFalse(log.contains("set-environment -g ICC_SURFACE_ID"), log)
        XCTAssertFalse(log.contains("set-environment -g ICC_PANEL_ID"), log)
    }

    func testShellIntegrationClearsStaleSurfaceScopedTmuxEnvironmentAutomatically() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("icc-zsh-tmux-clear-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        let logPath = root.appendingPathComponent("tmux.log", isDirectory: false)
        ...
        try writeExecutableScript(
            ...
            #!/bin/sh
            if [ "$1" = "show-environment" ] && [ "$2" = "-g" ]; then
              printf '%s\\n' 'ICC_SURFACE_ID=99999999-9999-9999-9999-999999999999'
              printf '%s\\n' 'ICC_PANEL_ID=99999999-9999-9999-9999-999999999999'
              exit 0
            fi
            printf '%s\\n' "$*" >> "\(logPath.path)"
            exit 0
        ...
        let log = (try? String(contentsOf: logPath, encoding: .utf8)) ?? ""
        XCTAssertTrue(log.contains("set-environment -gu ICC_SURFACE_ID"), log)
        XCTAssertTrue(log.contains("set-environment -gu ICC_PANEL_ID"), log)
    }

    func testShellIntegrationRefreshesWorkspaceScopedIccEnvironmentFromTmuxWithoutOverwritingSurfaceScope() throws {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("icc-zsh-tmux-refresh-\(UUID().uuidString)")
        let binDir = root.appendingPathComponent("bin", isDirectory: true)
        ...
        let output = try runInteractiveZsh(
            iccLoadGhosttyIntegration: false,
            iccLoadShellIntegration: true,
            command: "_icc_precmd; print -r -- \"$ICC_TAG|$ICC_SOCKET_PATH|$ICC_WORKSPACE_ID|$ICC_SURFACE_ID|$ICC_PANEL_ID\"",
            extraEnvironment: [
                "PATH": "\(binDir.path):/usr/bin:/bin:/usr/sbin:/sbin",
                "TMUX": "/tmp/tmux-stale,123,0",
                "ICC_SOCKET_PATH": "/tmp/icc-stale.sock",
                "ICC_TAG": "feat-tmux-integration-experiments",
                "ICC_WORKSPACE_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "ICC_SURFACE_ID": "22222222-2222-2222-2222-222222222222",
                "ICC_TAB_ID": "AAAAAAAA-AAAA-AAAA-AAAA-AAAAAAAAAAAA",
                "ICC_PANEL_ID": "22222222-2222-2222-2222-222222222222",
            ]
        )
        ...
        XCTAssertEqual(
            output,
            "feat-tmux-notification-attention-state|/tmp/icc-current.sock|11111111-1111-1111-1111-111111111111|22222222-2222-2222-2222-222222222222|22222222-2222-2222-2222-222222222222"
        )
    }
    ...
    func testShellIntegrationReportsTTYFromTmuxWithoutUsingPanelScope() throws {
        let output = try runInteractiveZsh(
            ...
            iccLoadShellIntegration: true,
            command: """
            _ICC_TTY_NAME=ttys999
            print -r -- "$(_icc_report_tty_payload)"
            """,
            extraEnvironment: [
    ...
    private func runInteractiveZsh(iccLoadGhosttyIntegration: Bool) throws -> String {
        try runInteractiveZsh(
            iccLoadGhosttyIntegration: iccLoadGhosttyIntegration,
            iccLoadShellIntegration: false,
    ...
    private func runInteractiveZsh(
        iccLoadGhosttyIntegration: Bool,
        iccLoadShellIntegration: Bool,
        command: String,
        extraEnvironment: [String: String] = [:]
    ) throws -> String {
        let fileManager = FileManager.default
        let root = fileManager.temporaryDirectory
            .appendingPathComponent("icc-zsh-shell-integration-\(UUID().uuidString)")
        ...
        let repoRoot = URL(fileURLWithPath: #filePath)
            ...
            .deletingLastPathComponent()
        let iccZdotdir = repoRoot.appendingPathComponent("Resources/shell-integration")
        ...
        process.environment = [
            ...
            "SHELL": "/bin/zsh",
            "USER": NSUserName(),
            "ZDOTDIR": iccZdotdir.path,
            "ICC_ZSH_ZDOTDIR": userZdotdir.path,
            "ICC_SHELL_INTEGRATION": "0",
            "GHOSTTY_RESOURCES_DIR": ghosttyResources.path,
        ]
        if iccLoadGhosttyIntegration {
            process.environment?["ICC_LOAD_GHOSTTY_ZSH_INTEGRATION"] = "1"
        }
        if iccLoadShellIntegration {
            process.environment?["ICC_SHELL_INTEGRATION"] = "1"
            process.environment?["ICC_SHELL_INTEGRATION_DIR"] = iccZdotdir.path
            process.environment?["ICC_SOCKET_PATH"] = root.appendingPathComponent("icc-test.sock").path
            process.environment?["ICC_TAB_ID"] = "tab-test"
            process.environment?["ICC_PANEL_ID"] = "panel-test"
        }
...
final class BrowserInstallDetectorTests: XCTestCase {
    ...
    private func makeTemporaryHome() -> URL {
        FileManager.default.temporaryDirectory.appendingPathComponent("icc-browser-detect-\(UUID().uuidString)")
    }
